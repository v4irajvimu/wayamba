$(document).ready(function(){
       ...
       $("#item_code").autocomplete('index.php/main/load_data/m_items/auto_com', {
        ...
        $(".fo").keypress(function(e){
            ...
            if(e.keyCode==13){
            $.post("index.php/main/load_data/t_price_change_batch_sum/get_item", {
                code:$("#0_"+scid).val(),
        ...
        $("#search_item").click(function(){
            clear_grid();
            $.post("index.php/main/load_data/t_price_change_batch_sum/get_item_detail",{
                code:$("#item_code").val(),
                date_to:$("#date_to").val(),
               ...
               if(r.data[0]!='0'){
                for(x=0;x<r.data.length;x++){
                    ...
                    $("#s3_"+x).val(r.data[x].sale_price_3);
                    $("#ns3_"+x).val(r.data[x].new_sale_price_3);
                    $("#s4_"+x).val(r.data[x].sale_price_4);
                    $("#ns4_"+x).val(r.data[x].new_sale_price_4);
                }
               }else{
    ...
    $("#btnDelete").click(function(){
        var id=$("#id").val();
            ...
            var id=$("#id").val();
             $.post("index.php/main/load_data/t_price_change_batch_sum/check_code",{
                id:id,
             },function(r){
...
function set_cus_values(f){
  ...
  function load_data(id){
    empty_grid();
    loding();
    $.post("index.php/main/get_data/t_price_change_batch_sum/", {
        id: id
    }, function(r){
            if(r=="2"){
               set_msg("No records");
            ...
            for(var i=0; i<r.det.length;i++){
                ...
                $("#6_"+i).val(r.det[i].sale_price_3);
                $("#p3_"+i).val(r.det[i].new_sale_price_3);
                $("#7_"+i).val(r.det[i].sale_price_4);
                $("#p4_"+i).val(r.det[i].new_sale_price_4);
                $("#nc_"+i).val(r.det[i].new_cost);
...
function set_delete(id){
    if(confirm("Are you sure delete "+id+"?")){
        loding();
        $.post("index.php/main/delete/t_price_change_batch_sum", {
            id : id
        }, function(res){
...
function load_items(){
    $.post("index.php/main/load_data/t_price_change_batch_sum/item_list_all", {
    ...
    $("#item_list .cl").click(function(){
        if($(this).children().eq(0).html() != "&nbsp;"){
            //if(check_item_exist($(this).children().eq(0).html())){
                ...
                var cost =parseFloat($(this).children().eq(4).html());
                var last =parseFloat($(this).children().eq(5).html());
                var max  =parseFloat($(this).children().eq(6).html());
                ...
                $("#bt_"+scid).val($(this).children().eq(3).html());
                
                $("#2_"+scid).val($(this).children().eq(4).html());
                $("#22_"+scid).val(last_precentage);
                $("#3_"+scid).val($(this).children().eq(5).html());
                $("#5_"+scid).val($(this).children().eq(6).html());
                $("#55_"+scid).val(max_precentage);

                $("#6_"+scid).val($(this).children().eq(7).html());
                $("#7_"+scid).val($(this).children().eq(8).html());
