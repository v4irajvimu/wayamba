...
function load_cheque(){
    empty_grid();
    $.post("index.php/main/load_data/t_cheque_issue_trans/load_cheques", {
        ...
        if(r != 2){
            ...
            for(var i=0; i<r.length; i++){
              ...
              $("#n_"+i).val(r[i].acc_name);             
              $("#1_"+i).val(r[i].to_acc_code);
              $("#1_name_"+i).val(r[i].to_acc_name);
...
function load_acc(){
        $.post("index.php/main/load_data/t_cheque_issue_trans/acc_list", {
...
function set_delete(){
    ...
    if(id != 0){
        if(confirm("Are you sure delete cheque deposit no? "+id)){
            loding();
            $.post("index.php/main/delete/t_cheque_issue_trans", {
                id : id
            }, function(r){
                if(r != 1){
                    loding();
...
function load_data(id){
    empty_grid();
    $.post("index.php/main/get_data/t_cheque_issue_trans/", {
        id : id
    }, function(r){
        if(r!= 2){
            ...
            $("#tot_balance").val(0);
            $("#total").val(r.sum[0].settle);
            $("#tot_settle").val(r.sum[0].settle);
            
            for(var x=0; x<r.det.length; x++){
                $("#0_"+x).val(r.det[x].account);
                $("#n_"+x).val(r.det[x].acc_name);
                $("#1_"+x).val(r.det[x].to_acc_code);
                $("#1_name_"+x).val(r.det[x].to_acc_name);
...
function validate(){
    ...
    if($("#tot_settle").val() ==0 || $("#tot_settle").val()==""){
        set_msg("Please Select Cheques For Withdraw !");
        v = false;
    }else{
        v = true;
    }    
