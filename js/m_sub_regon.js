...
function check_delete_permission(code)
{
    ...
    if(r>0)
    {
        ...
        if(confirm("Are you sure delete "+code+"?")){
          loding();
          $.post("index.php/main/delete/m_sub_regon", {
              code : code
          }, function(res){
              if(res == 1){
                  location.reload();
...
function check_code(){
    var code = $("#code").val();
    $.post("index.php/main/load_data/m_sub_regon/check_code", {
        code : code
    }, function(res){
        if(res == 1){
...
function validate(){
    if($("#code").val() === $("#code").attr('title') || $("#code").val() == ""){
            ...
            $("#des").focus();
            return false;
        }else if($("#main_region option:selected").attr('value') == "0"){
            alert("Please select Main Region.");
            return false;
        }else{
            return true;
        }
...
function set_edit(code){
    loding();
    $.post("index.php/main/get_data/m_sub_regon", {
        code : code
    }, function(res){
        $("#code_").val(res.code);
        $("#code").val(res.code);
        ...
        $("#main_region").val(res.main_region);
        set_select('main_region', 'mr_des');
        loding(); input_active();
        is_edit=1;
    }, "json");
