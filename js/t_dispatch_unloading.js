...
$(document).ready(function(){
  ...
  $(".qunsb").click(function(){
    ...
    $("#btnDelete").click(function () {
        ...
        loding();
        $.post("index.php/main/load_data/t_dispatch_unloading/checkdelete", {
            no: $("#id").val(),
        },
        function (r) {
         loding();
            if (r == 2) {
                ...
                var no = $("#id").val();
             loding();
                if (confirm("Are you sure canel " + no + "?")) {

                    $.post("index.php/main/delete/t_dispatch_unloading", {
                        id: no,
                    },
                    function (r) {
                        if(r==1)
   ...
   $("#btnExit1").click(function(){
    ...
    $("#officer").autocomplete('index.php/main/load_data/t_dispatch_unloading/auto_com', {
        width: 350,
        multiple: false,
        matchContains: true,
        formatItem: formatItems,
...
function load_items(){
    $.post("index.php/main/load_data/t_dispatch_unloading/item_list_all", {
...
function validate(){
  ...
  if (code==""||code==null || save_chk=="1") {
      ...
      return false;
  }else if($("#from_store").val()!=0 && $("#group_id").val()==""){
...
function load_data(id){
    var g=[];
    empty_grid();
    $.post("index.php/main/get_data/t_dispatch_unloading/", {
        id: id,
...
function load_items3(x){
    $.post("index.php/main/load_data/t_dispatch_unloading/batch_item", {
        search : x,
        stores : $("#from_store").val()
    }, function(r){
        $("#sr3").html(r);
...
function check_is_batch_item(scid){
        ...
        $.post("index.php/main/load_data/t_dispatch_unloading/is_batch_item",{
            code:$("#0_"+scid).val(),
            store:store
         },function(res){
