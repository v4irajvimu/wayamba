...
var request_id = 0; var count = 100; var root; var sroot = sarea = trans_codi = storse = driver = 0; 
$(document).ready(function(){
    
    
    enter_setup_trance('tgrid2');
    
    $("#trans_codi").val(trans_codi); $("#stores").val(storse);
    set_select('trans_codi', 'trans_des'); set_select('stores', 'sto_des');
    set_select('mar_codi', 'mar_des');
    set_select('driver', 'driver_des');
    
    $("#sale").change(function(){
        if($("#customer").val()=='' || $("#customer").val()=='0')
        {
...
//    });
    ...
    $("#btnPrint").click(function(){
        var id = $("#hid").val()
        if(id != 0){
            window.open('index.php/prints/trance_forms/t_dispatch_note/?id='+id);
        }else{
    ...
    $("#mar_codi").change(function(){
        set_select('mar_codi', 'mar_des');
    });

    $("#trans_codi").change(function(){
    ...
    $("#btnReset").click(function(){
       $("#btnReset").removeAttr("disabled", "disabled");
...
function check_is_dipatch_return(id){
   
    $.post("index.php/main/get_subitem_data/t_dispatch_note/", {
        id : id
    }, function(r){
            load_items();
            ...
            if(check_item_exist($(this).children().eq(0).html())){
                if(parseFloat($(this).children().eq(5).html()) > 0){
                   ...
                   // load_subitem($(this).children().eq(0).html(),scid)
                      load_current_stock($(this).children().eq(0).html(),scid); 
...
//    }, "json");
//}
function load_current_stock(id,scid){
   
    $.post("index.php/main/load_data/t_dispatch_note/get_current_stock", {
        id : id
    }, function(r){
            load_items();
...
function load_address(){
    ...
    if(code != "0"){
        $.post("index.php/main/load_data/m_customer/get_address", {
            code : code
        }, function(r){
...
function validate(){
    ...
    if(v == false){
        ...
        v = false;
    }else if($("#trans_codi option:selected").val() == 0){
        alert("Please select a Transaction Coordinator");
        v = false;
    }else if($("#mar_codi option:selected").val() == 0){
        alert("Please select a Marketing Coordinator");
        v = false;
    }else if($("#driver option:selected").val() == 0){
...
function save(){
	...
	$.ajax({
		...
		if(! isNaN(pid)){
			...
			window.open('index.php/prints/trance_forms/t_dispatch_note/?id='+pid, '_blank');
			window.open('?action=t_dispatch_note', '_self');
		    }else{
			window.open('?action=t_dispatch_note', '_self');
		    }
		}else{
