...
$(function(){
    ...
    if(p_code != undefined){
        load_data(p_code);
        load_payment_option_data(p_code,"69");
    }
    ...
    $("#showPayments").click(function(){
      payment_opt('t_cash_and_card_sales_sum',$("#net").val());
    ...
    $("#btnResett").click(function(){
        location.href="?action=t_cash_and_card_sales_sum";
    });
    ...
    $(".fo").keypress(function(e){  
        ...
        if(e.keyCode==13){
            $.post("index.php/main/load_data/t_cash_and_card_sales_sum/get_item", {
                code:$("#0_"+scid).val(),
...
/*        $("#sales_rep").autocomplete('index.php/main/load_data/m_employee/auto_com', {
        ...
        $("#id").keypress(function(e){
            if(e.keyCode == 13){
                $(this).blur();
                load_data($(this).val());
                load_payment_option_data($(this).val(),"69");
                $("#btnSave").attr("disabled","disabled");
            }
        });
...
function sales_category_max(){
  $.post("index.php/main/load_data/utility/get_max_sales_category", {
    nno:"sub_no",
    table:"t_cash_and_card_sales_sum",
...
$.post("index.php/main/load_data/t_cash_and_card_sales_sum/item_free_delete",{
    quantity:qty,
    item:item
},function(r){
...
$.post("index.php/main/load_data/t_cash_and_card_sales_sum/item_free",{
    quantity:qty,
    item:item,
    date:$("#date").val()
},function(r){
...
function check_is_batch_item(scid){
    ...
    $.post("index.php/main/load_data/t_cash_and_card_sales_sum/is_batch_item",{
        code:$("#0_"+scid).val(),
        store:store
    },function(res){
...
function load_items3(x){
    $.post("index.php/main/load_data/t_cash_and_card_sales_sum/batch_item", {
        search : x,
        stores : $("#stores").val()
    }, function(r){
        $("#sr3").html(r);
...
$.post("index.php/main/load_data/t_cash_and_card_sales_sum/item_free_list",{
    quantity:qty,
    item:item,
    date:$("#date").val()
},function(r){   
...
function save(){
    ...
    $('#form_').attr('action',$('#form_id').val()+"t_cash_and_card_sales_sum");
    var frm = $('#form_');
...
function check_code(){
    loding();
    var code = $("#code").val();
    $.post("index.php/main/load_data/t_cash_and_card_sales_sum/check_code", {
        code : code
    }, function(res){
        if(res == 1){
...
function set_delete(){
    var id = $("#hid").val();
    if(id != 0){
        if(confirm("Are you sure to delete this cash and card sale ["+$("#hid").val()+"]? ")){
            $.post("index.php/main/delete/t_cash_and_card_sales_sum", {
                trans_no:id,
            },function(r){
              if(r != 1){
                set_msg(r);
...
function load_items(){        
   $.post("index.php/main/load_data/t_cash_and_card_sales_sum/item_list_all", {
    ...
    $("#item_list .cl").click(function(){
        if($(this).children().eq(0).html() != "&nbsp;"){
            if(check_item_exist($(this).children().eq(0).html())){
                if(parseInt($(this).children().eq(6).html())>0){
                    if($("#df_is_serial").val()=='1'){
                    ...
                    set_msg("Item "+$(this).children().eq(1).html()+" not enough quantity.")
                }
            }else{
                set_msg("Item "+$(this).children().eq(1).html()+" is already added.");
            }
...
function load_data(id){
    ...
    empty_grid();
    loding();
    $.post("index.php/main/get_data/t_cash_and_card_sales_sum/", {
        id: id
    }, function(r){
...
/*    function load_data9(){
        $.post("index.php/main/load_data/t_cash_and_card_sales_sum/customer_list", {
    ...
    function load_data9(){
       // $.post("index.php/main/load_data/t_cash_and_card_sales_sum/f1_selection_list_customer", {
    ...
    function load_data10(){
        $.post("index.php/main/load_data/t_cash_and_card_sales_sum/load_b_foc", {
            date:$("#date").val(),
            search : $("#pop_search").val() 
        }, function(r){
            $("#sr").html(r);
    ...
    function load_foc_items(code,date){
        loding();
        empty_grid();
        $.post("index.php/main/load_data/t_cash_and_card_sales_sum/load_foc_items", {
            code:code,
            date:date
        }, function(r){
            loding();
...
function check_batch_qty(scid){
    $.post("index.php/main/load_data/t_cash_and_card_sales_sum/get_batch_qty",{
        store:$("#stores").val(),
...
function default_customer_details(){
   $.post("index.php/main/load_data/t_cash_and_card_sales_sum/load_default_customer",{
    cluster : $("#cl").val(),
    branch : $("#bc").val()
}, function(r){
    if(r!=2)
    {
        ...
        var address=""; 
        if (r[0].address1!="") {address+=r[0].address1};
        if (r[0].address2!="") {address+=", "+r[0].address2};
        if (r[0].address3!="") {address+=", "+r[0].address3};
        $("#address").val(address);
