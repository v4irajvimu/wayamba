...
$(document).ready(function(){
    ...
    $("#btnPrint").click(function(){
        var id = $("#hid").val()
        if(id != 0){
            window.open('index.php/prints/trance_forms/t_stock_adjustment/?id='+id);
        }else{
...
function make_balance(f){
    var a = parseFloat(f.val());
    if(isNaN(a)){ a = 0; }
    
    var s = parseFloat(f.parent().parent().children().eq(2).children().val());
    var p = parseFloat(f.parent().parent().children().eq(5).children().val());
    ...
    f.parent().parent().children().eq(4).children().val(b);
    f.parent().parent().children().eq(6).children().val(b2);
}
...
function set_delete(){
    ...
    if(id != 0){
        if(confirm("Are you sure ? ")){
            $.post("index.php/main/delete/t_stock_adjustment", {
                id : id
            }, function(r){
                if(r != 1){
...
function load_data(id){
    empty_grid();
    $.post("index.php/main/get_data/t_stock_adjustment/", {
        id : id
    }, function(r){
        if(r.sum.id != undefined){
            ...
            for(var i=0; i<r.det.length; i++){
                ...
                if(r.det[i].is_measure == 1){
                    $("#2_"+i).val(r.det[i].actual);
                    $("#2_"+i).autoNumeric({mDec:2});
                }else{
                    $("#2_"+i).val(parseInt(r.det[i].actual));
                    $("#2_"+i).autoNumeric({mDec:0});
                }
                
                $("#1_"+i).val(r.det[i].system);
                $("#3_"+i).val(r.det[i].balance);
                $("#4_"+i).val(r.det[i].purchase);
                $("#5_"+i).val(r.det[i].value);
