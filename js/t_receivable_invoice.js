...
$(document).ready(function () {
    ...
    $(".fo").keypress(function (e) {
        ...
        if (e.keyCode == 13) {
            $.post("index.php/main/load_data/t_receivable_invoice/get_account", {
                code: $("#0_" + scid).val()
            }, function (res) {
                if (res.a != 2) {
    ...
    $("#sjournal_type").autocomplete('index.php/main/load_data/r_journal_type/auto_com_receivable', {
        width: 350,
        multiple: false,
        matchContains: true,
...
function set_delete() {
    ...
    if (id != 0) {
        if (confirm("Are you sure ? ")) {
            $.post("index.php/main/delete/t_receivable_invoice", {
                id: id
            }, function (r) {
                if (r != 1) {
                    set_msg(r, "error");
...
function load_data(id) {
    empty_grid();
    $.post("index.php/main/get_data/t_receivable_invoice/", {
        id: id
    }, function (r) {
        if (r.sum.no != undefined) {

            $("#date").val(r.sum.date);
            $("#journal_type").val(r.sum.receivable_type);
            $("#sjournal_type").val(r.sum.receivable_type);
            $('#journal_type_des').val(r.sum.receivable_type_des);
            $("#description").val(r.sum.receivable_des);
            $("#ref_no").val(r.sum.ref_no);
            $("#narration").val(r.sum.narration);
            $("#saccount").val(r.sum.receivable_account);
            $("#account").val(r.sum.receivable_account);
            $("#account_des").val(r.sum.receivable_account_des);
...
function validate() {
    ...
    if (v == false) {
        ...
        alert("Please select receivable account");
        v = false;
    }
    else if ($("#description").val() == "") {
        alert("Please enter description");
...
function get_max() {
    $.post("/index.php/main/load_data/t_receivable_invoice/get_max", {
    }, function (r) {
        $("#id").val(r.max_no);
    }, "json");
}
