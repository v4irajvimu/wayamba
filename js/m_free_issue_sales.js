$(document).ready(function(){
    $("#tgrid").tableScroll({height:400, width:435});
    $("#tgrid2").tableScroll({height:400, width:435});
...
function load_data(){
    empty_grid();
	$.post("index.php/main/get_data/m_free_issue_sales/", {
        no: $("#no").val()
    }, function (r) {  
        if(r==2){
            set_msg("No Records ! ! !");
...
function load_foc() {
    ...
    $.post("index.php/main/load_data/m_free_issue_sales/foc_sale_list", {
        search: $("#pop_search").val(),
    }, function (r) {
        $("#sr").html(r);
        settings1();
...
function set_delete(){
    ...
    if(id != 0){
        if(confirm("Are you sure to delete this bulk FOC ["+$("#hid").val()+"]? ")){
            $.post("index.php/main/delete/m_free_issue_sales", {
            id:id,
        },function(r){
          if(r != 1){
            set_msg(r);
