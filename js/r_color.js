...
function get_data_table(){
    $.post("index.php/main/load_data/r_color/get_data_table", {
...
function check_code(){
    var code = $("#code").val();
    $.post("index.php/main/load_data/r_color/check_code", {
        code : code
    }, function(res){
        if(res == 1){
...
function validate(){
    if($("#code").val() == ""){
       ...
       /* }else if($("#code").val().length<4){
            ...
            $("#code_gen").focus();
            return false;
        }else if($("#code_gen").val().length<3){
            set_msg("Code must contains 3 charactors","error");
            $("#code_gen").focus();
            return false;
...
if(confirm("Are you sure delete "+code+"?")){
        loding();
        $.post("index.php/main/delete/r_color", {
            code : code
        }, function(res){
            if(res == 1){
                 loding();
...
function set_edit(code){
   loding();
    $.post("index.php/main/get_data/r_color", {
        code : code
    }, function(res){
