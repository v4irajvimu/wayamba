...
$(function(){
    ...
    if(p_code != undefined){
        load_data(p_code);
        load_payment_option_data(p_code,"70");
    }
    setTimeout("default_customer_details();", 100);
    defult_sales_order_store();
    ...
    $("#showPayments").click(function(){
      payment_opt('t_sales_order_sales_sum',$("#net").val());
    ...
    $("#btnResett").click(function(){
        location.href="?action=t_sales_order_sales_sum";
    });
    ...
    $("#sales_order_no").keypress(function(e){
        if(e.keyCode == 112){
            $("#pop_search14").val($("#sales_order_no").val());
            load_sales_order();
            $("#serch_pop14").center();
            $("#blocker").css("display", "block");
    ...
    $("#sales_order_no").keypress(function(e){
        if(e.keyCode==13){
            if($("#sales_order_no").val()!="" && $("#stores").val()!="0"){
                load_sales_order_details($("#sales_order_no").val());
                load_sales_order_additional_details($("#sales_order_no").val());
            }else{
                set_msg("please select sales order no and store ");
            }
        }
    });
    ...
    $(".fo").keypress(function(e){  
        set_cid($(this).attr("id"));
           ...
           $("#pop_search").keyup(function(e){
            ...
            if(e.keyCode==13){
                $.post("index.php/main/load_data/t_sales_order_sales_sum/get_item", {
                    code:$("#0_"+scid).val(),
            ...
            $("#soqty_"+scid).attr('style', 'background-color: #f9f9ec !important; width : 100%; text-align:right;');
            $("#soqty_"+scid).closest("td").attr('style', 'width: 58px; background-color: #f9f9ec !important');
...
$(".qunsb").click(function(){
    ...
    */
    ...
    $(".qun").keyup(function(){
        set_cid($(this).attr("id"));
        if($("#soqty_"+scid).val()==$("#5_"+scid).val() || $("#soqty_"+scid).val()>$("#5_"+scid).val()){
        }
        else{
            set_msg("Please Enter Low qty above SO qty");
            $("#5_"+scid).val("");
        }
    });
...
/*        $("#sales_rep").autocomplete('index.php/main/load_data/m_employee/auto_com', {
        ...
        $("#id").keypress(function(e){
            if(e.keyCode == 13){
                $(this).blur();
                load_data($(this).val());
                load_payment_option_data($(this).val(),"70");
                $("#btnSave").attr("disabled","disabled");
            }
        });
...
function load_price_range(item,batch){
    if(item!="" && batch!=""){
        load_prices($("#0_"+scid).val(),$("#1_"+scid).val());
        $("#serch_pop11").center();
        $("#blocker").css("display", "block");
    }else{
...
function load_prices(item,batch){
    $.post("index.php/main/load_data/t_sales_order_sales_sum/sales_prices", {
        item:item,
        batch:batch
    }, function(r){
        $("#sr11").html(r);
           ...
           var price=Item_price;
            ...
            for(var i=0; i<25 ;i++){
                if($("#0_"+i).val()==get){
                    ...
                    $("#soqty_"+i).attr("readonly", false);
                    $("#soqty_"+i).closest("td").removeClass('g_col_fixed').attr('style', 'width: 71px; background-color: rgb(224, 228, 146) !important');
                    $("#soqty_"+i).removeClass('g_col_fixed').attr('style', 'background-color: rgb(224, 228, 146) !important; margin:0;padding:0;width :56px; float: right; text-align:right;');
...
function load_sales_order_additional_details(so_no){

    $.post("index.php/main/load_data/t_sales_order_sales_sum/load_sales_order_additional_details", {
        sales_no:so_no
    }, function(r){
        if(r==2){
...
// alert(r[i].is_add);
$("#hh_"+i).val(r[i].is_add);
$("#hhh_"+i).val(r[i].type);
$("#00_"+i).val(r[i].type);
$("#nn_"+i).val(r[i].description);
$("#11_"+i).val(r[i].rate_p);
$("#tt_"+i).val(r[i].amount);
addit_amount_calc()
}
}          
},"json");
...
function addit_amount_calc(){
    ...
    if (isNaN(amc)) {amc=0;};
    $("#addit_amount").val(amc);

}


function load_sales_order_details(so_no){
    empty_grid();

    $.post("index.php/main/load_data/t_sales_order_sales_sum/load_sales_order_details", {
        sales_no:so_no
    }, function(r){
        if(r==2){
            set_msg("no records ! ! ! ")
        }else{
            //$("#stores").val(r[0].store);
            $("#customer").val(r[0].cus_id);
            $("#customer_id").val(r[0].name);
            $("#address").val(r[0].address);
            $("#sales_rep").val(r[0].rep_id);
            $("#sales_rep2").val(r[0].rep_name);

            for(var x=0; x<r.length; x++){
                $("#0_"+x).val(r[x].code);
                if(check_item_exist($("#0_"+x).val())){
                    if($("#df_is_serial").val()=='1'){
                        check_is_serial_item2(r[x].code,x);
                    }
                    check_is_batch_item2(r[x].code,x);
                    check_is_sub_item2(r[x].code,x);
                    $("#h_"+x).val(r[x].code);
                    $("#n_"+x).val(r[x].description);
                    $("#0_"+x).val(r[x].code);
                    $("#soqty_"+x).val(r[x].qty);
                    $("#2_"+x).val(r[x].model);
                    $("#7_"+x).val(r[x].discount);
                    $("#3_"+x).val(r[x].max_price);
                    $("#item_min_price_"+x).val(r[x].min_price);
                    $("#free_price_"+x).val(r[x].max_price); 
                    $("#cost_"+x).val(r[x].purchase_price);
                    $("#1_"+x).val(r[x].batch_no);


                // $("#hhh_"+x).val(r[x].sales_type);
                // $("#00_"+x).val(r[x].sales_type);
                // $("#nn_"+x).val(r[x].description);
                // $("#11_"+x).val(r[x].rate_p);
                // $("#tt_"+x).val(r[x].amount);


                $("#pop_close").click();
                    //check_is_batch_item(x);
                }else{
                    set_msg("Item "+$("#0_"+x).val()+" is already added.","error");
                }
            }
...
$.post("index.php/main/load_data/t_sales_order_sales_sum/item_free_delete",{
    quantity:qty,
    item:item
},function(r){
        ...
        for(var x=0; r.a.length>x;x++){
            for(var i=0; i<25;i++){ 
                if($("#0_"+i).val()==item || $("#0_"+i).val()==r.a[x].code  && $("#f_"+i).val()==1){
                    ...
                    $("#soqty_"+i).attr('style', 'background-color: #f9f9ec !important; width : 100%; text-align:right;');
                    $("#soqty_"+i).closest("td").attr('style', 'width: 58px; background-color: #f9f9ec !important');
...
$.post("index.php/main/load_data/t_sales_order_sales_sum/item_free",{
    quantity:qty,
    item:item,
    date:$("#date").val()
},function(r){
    if(r!='2')
    {
        ...
        for(i=0; i<r.a.length; i++)
        {
            if(r.a[i].code == item)
            {
                var free_qty=parseInt(r.a[i].item_qty)
                var select_qty=parseInt(r.a[i].qty)
                var issue=qty/select_qty;
                

                $("#5_"+no).val(Math.floor(issue)+qty);
                $("#4_"+no).val(Math.floor(issue));
            }
        }
...
function check_is_batch_item(scid){
    ...
    $.post("index.php/main/load_data/t_sales_order_sales_sum/is_batch_item",{
        code:$("#0_"+scid).val(),
        store:store
    },function(res){
...
function load_items3(x){
    $.post("index.php/main/load_data/t_sales_order_sales_sum/batch_item", {
        search : x,
        stores : $("#stores").val()
    }, function(r){
        $("#sr3").html(r);
...
$.post("index.php/main/load_data/t_sales_order_sales_sum/item_free_list",{
    quantity:qty,
    item:item,
    date:$("#date").val()
},function(r){   
...
function save(){
    ...
    $('#form_').attr('action',$('#form_id').val()+"t_sales_order_sales_sum");
    var frm = $('#form_');
...
function check_code(){
    loding();
    var code = $("#code").val();
    $.post("index.php/main/load_data/t_sales_order_sales_sum/check_code", {
        code : code
    }, function(res){
        if(res == 1){
...
function set_delete(){
    var id = $("#hid").val();
    if(id != 0){
        if(confirm("Are you sure to delete this sales order sale ["+$("#hid").val()+"]? ")){
            $.post("index.php/main/delete/t_sales_order_sales_sum", {
                trans_no:id,
...
function load_items(){        
   $.post("index.php/main/load_data/t_sales_order_sales_sum/item_list_all", {
...
function sales_order_settings(){
    $("#item_list .cl").click(function(){        
        $("#sales_order_no").val($(this).children().eq(0).html());
        load_sales_order_details($("#sales_order_no").val());
        load_sales_order_additional_details($("#sales_order_no").val());
        $("#pop_close14").click(); 
    })    
...
function load_data(id){
    ...
    empty_grid();
    loding();
    $.post("index.php/main/get_data/t_sales_order_sales_sum/", {
        id: id
    }, function(r){
        ...
        if(r=="2"){
         set_msg("No records");
           ...
           $("#sales_order_no").val(r.sum[0].sales_order_no);
...
function load_sales_order(){
    $.post("index.php/main/load_data/t_sales_order_sales_sum/sales_order_list", {
...
function load_data10(){
    $.post("index.php/main/load_data/t_sales_order_sales_sum/load_b_foc", {
        date:$("#date").val(),
        search : $("#pop_search").val() 
    }, function(r){
        $("#sr").html(r);
...
function load_foc_items(code,date){
    loding();
    empty_grid();
    $.post("index.php/main/load_data/t_sales_order_sales_sum/load_foc_items", {
        code:code,
        date:date
    }, function(r){
        loding();
...
function empty_grid(){
    for(var i=0; i<25; i++){
     ...
     $("#n_"+i).closest("td").attr('style', 'width:200px; background-color: #f9f9ec !important');
     $("#2_"+i).attr('style', 'width: 100%; background-color: #f9f9ec !important');
     $("#1_"+i).closest("td").attr('style', 'width: 71px; background-color: #f9f9ec !important');
     $("#1_"+i).attr('style', 'margin:0;padding:0;width :56px; float: right; text-align:right; background-color: #f9f9ec !important');
     $("#3_"+i).closest("td").attr('style', 'width: 58px;');
...
function check_batch_qty(scid){
    $.post("index.php/main/load_data/t_sales_order_sales_sum/get_batch_qty",{
        store:$("#stores").val(),
...
function default_customer_details(){
    $.post("index.php/main/load_data/t_sales_order_sales_sum/load_default_customer",{
        cluster : $("#cl").val(),
        branch : $("#bc").val()
    }, function(r){
        if(r!=2)
...
function defult_sales_order_store(){
    $.post("index.php/main/load_data/t_sales_order_sales_sum/sales_order_store",{
    }, function(r){
       //alert(r);
       $("#stores").val(r);
       set_select('stores','store_id');
