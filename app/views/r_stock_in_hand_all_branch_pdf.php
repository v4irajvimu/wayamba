		...
		$this->pdf->AddPage("P","A4");   // L or P amd page type A4 or A3
 		...
 		foreach($branch as $ress){//Common
        	...
        	$this->pdf->SetFont('helvetica', 'IBU',12);
		 	$this->pdf->Cell(0, 5, 'Stock In Hand - All Branch   ',0,false, 'L', 0, '', 0, false, 'M', 'M');
			...
			$item_des="";
	 		foreach($r_st_i_h as $res){
				...
				$this->pdf->Ln();
			 	...
			 	$this->pdf->Cell(12, 6,"Item : ", '0', 0, 'L', 0);
			 	$this->pdf->Cell(0, 6,$item." - ".$item_des, '0', 0, 'L', 0);
				$this->pdf->Ln();
				$this->pdf->Ln();

			 	$this->pdf->Cell(30, 6,"Branch Code", '1', 0, 'C', 0);
			...
			$tot_amo=0;
	 		...
	 		foreach($r_st_i_h as $res){
			        ...
			        $this->pdf->MultiCell(30, 6, $res->bc, $border=1, $align='L', $fill=false, $ln=0, $x=($this->pdf->GetX()), $y=$this->pdf->GetY(), $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=6, $valign='M', $fitcell=true);
			        $this->pdf->MultiCell(40, 6, $res->br_name, $border=1, $align='L', $fill=false, $ln=0, $x=($this->pdf->GetX()), $y=$this->pdf->GetY(), $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=6, $valign='M', $fitcell=true);
