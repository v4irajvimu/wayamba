<?php
		...
		$orgin_print=$_POST['org_print'];
	  	...
	  	$this->pdf->Cell(0, 5,' SEETTU HISTORY',0,false, 'C', 0, '', 0, false, 'M', 'M');
		...
		foreach($det as $row){
			...
			if($x==1){
	            ...
	            $item_cat1=$row->item_cat;

				$this->pdf->SetFont('helvetica','',9);

				$this->pdf->MultiCell(15, $heigh, $a, 1, 'R', 0, 0, '', '', true, 0, false, true, 0);
				$this->pdf->MultiCell(30, $heigh, $row->item_cat, 1, 'R', 0, 0, '', '', true, 0, false, true, 0);
			...
			if($x==2){
				...
				$c_item_code=$row->code;

				$this->pdf->Ln();
				$this->pdf->SetX(31);
	        	$this->pdf->SetLineStyle(array('width' => 0.1, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0.1, 'color' => array(0, 0, 0)));
				$this->pdf->SetFont('helvetica','B',10);
	        	$this->pdf->Cell(40, 6, "Category Code :	".$c_item_code, '0', 0, 'L', 0);
	        	$this->pdf->Ln();
		        ...
		        $c_item_code=$row->code;
				if($c_item_code==$item_cat1){

					 $this->pdf->SetX(31);
					//$this->pdf->GetY(60);
			    	$this->pdf->SetFont('helvetica','',9);
			        $this->pdf->MultiCell(15, $heigh, $z, 1, 'R', 0, 0, '', '', true, 0, false, true, 0);
				 	$this->pdf->MultiCell(30, $heigh, $row->item_code, 1, 'R', 0, 0, '', '', true, 0, false, true, 0);
			...
			else if($c_item_code!=$item_cat1){
				$this->pdf->Ln();
				$this->pdf->SetFont('helvetica','B',9);
				$this->pdf->MultiCell(167, $heigh, "No. of Items :	".$a, 0, 'R', 0, 1, '', '', true, 0, false, true, 0);
