<?php if($this->user_permissions->is_view('018')){ ?>
<link rel="stylesheet" type="text/css" href="<?=base_url()?>css/t_forms.css" />
<script type="text/javascript" src="<?=base_url()?>js/t_purchase_order.js"></script>
<?php
if(isset($_GET['print'])){
...
<script type="text/javascript">
    $(document).ready(function(){
        if(confirm("Do you need get print?")){
            window.open('index.php/prints/trance_forms/t_purchase_order/?id='+pid, '_blank');
            window.open('?action=t_purchase_order', '_self');
        }else{
            window.open('?action=t_purchase_order', '_self');
        }
    });
...
<div class="dframe" id="mframe">
    <form method="post" action="<?=base_url()?>index.php/main/save/t_purchase_order" id="form_">
