<?php
    ...
    $this->pdf->Cell(180, 1,"Payable Invoice - Details   ",0,false, 'L', 0, '', 0, false, 'M', 'M');
    $this->pdf->Ln();
   ...
   $this->pdf->Cell(20, 4,"$bc_id - $branch_name", '0', 0, 'L', 0);
        ...
        foreach($r_payable_invoice_details as $row){
         ...
         if($no==0) 
         {
            ...
            $this->pdf->SetFont('helvetica','B',9);
             ...
             $aa = $this->pdf->getNumLines($row->des, 85);
             $bb=$this->pdf->getNumLines($row->acc_name, 55); 
            ...
            if($bb>$aa)
              ...
              $heigh=5*$aa;
               ...
               $this->pdf->MultiCell(40, $heigh, $row->p_type,  1, 'L', 0, 0, '', '', true, 0, false, true, 0);
               $this->pdf->MultiCell(85, $heigh, $row->des,  1, 'L', 0, 0, '', '', true, 0, false, true, 0);
               $this->pdf->MultiCell(25, $heigh, $row->payable_account,  1, 'L', 0, 0, '', '', true, 0, false, true, 0);
               $this->pdf->MultiCell(55, $heigh, $row->acc_name,  1, 'L', 0, 0, '', '', true, 0, false, true, 0);
              ...
              if($bb>$aa)
               ...
               $this->pdf->MultiCell(55, $heigh, $row->acc_dname, 1, 'L', 0, 0, '', '', true, 0, false, true, 0);
    ...
    //----------------------------------------------------------
        ...
        $this->pdf->Output("payable_invoice_summery".date('Y-m-d').".pdf", 'I');
