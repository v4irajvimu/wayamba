<?php if($this->user_permissions->is_view('023')){ ?>
...
<script type="text/javascript" src="<?=base_url()?>js/t_supplier_settlement.js"></script>
<?php
if(isset($_GET['print'])){
...
<script type="text/javascript">
    $(document).ready(function(){
        if(confirm("Do you need get print?")){
            window.open('index.php/prints/trance_forms/t_supplier_settlement/?id='+pid, '_blank');
            window.open('?action=t_supplier_settlement', '_self');
        }else{
            window.open('?action=t_supplier_settlement', '_self');
        }
    });
...
<div class="dframe" id="mframe">
    <form method="post" action="<?=base_url()?>index.php/main/save/t_supplier_settlement" id="form_">
        <table style="width: 100%" border="0">
            <tr>
                ...
                <td><input type="text" class="input_date_down_future" readonly="readonly" name="date" id="date" title="<?=date('Y-m-d')?>" style="text-align: right; width: 100px;"/></td>
            </tr><tr>
                <td>Balance</td>
                <td>
