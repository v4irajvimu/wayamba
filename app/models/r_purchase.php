...
class r_purchase extends CI_Model {
    ...
    //         $sql = "SELECT
    //                     t_purchse_sum.no,
    //                     m_supplier.full_name,
    //                     IF(t_purchse_sum.invoice_no = 'Invoice No', '&nbsp;', t_purchse_sum.invoice_no) AS invoice_no,
    //                     m_stores.description,
    //                     (pri.cost)-discount AS cost
    //                   FROM t_purchse_sum
    //                     INNER JOIN m_supplier
    //                       ON (m_supplier.code = t_purchse_sum.supplier)
    //                     INNER JOIN m_stores
    //                       ON (m_stores.code = t_purchse_sum.stores)
    //                     LEFT OUTER JOIN (SELECT
    //                                        SUM((quantity*cost)-t_purchse_det.discount) AS cost,
    //                                        id
    //                                      FROM t_purchse_det
    //                                      GROUP BY id) AS pri
    //                       ON (pri.id = t_purchse_sum.id)
    //                   WHERE t_purchse_sum.date BETWEEN '".$this->sd['from']."'
    //                       AND '".$this->sd['to']."'
    //                       AND t_purchse_sum.is_cancel = 0
    //                       AND bc = '".$this->sd['bc']."'
    //                   ORDER BY t_purchse_sum.id";
    //     }else{
    //         $sql = "SELECT
    //                     t_purchse_sum.no,
    //                     full_name,
    ...
    //                     t_purchse_det.cost-t_purchse_det.discount AS cs,
    //                     t_purchse_sum.discount AS dis
                        
    //                   FROM t_purchse_det
    //                     INNER JOIN t_purchse_sum
    //                       ON (t_purchse_sum.id = t_purchse_det.id)
    //                     INNER JOIN m_supplier
    //                       ON (m_supplier.code = t_purchse_sum.supplier)
    //                     INNER JOIN m_items
    //                       ON (m_items.code = t_purchse_det.item_code)
    //                   WHERE t_purchse_sum.date BETWEEN '".$this->sd['from']."'
    //                       AND '".$this->sd['to']."'
    //                       AND t_purchse_sum.is_cancel = 0
    //                       AND bc = '".$this->sd['bc']."'
    ...
    //             $vehi = array("data"=>"Invoice No","style"=>"text-align: left; ", "chalign"=>"text-align: left; width:100px");
    ...
    //             $vehi  = array("data"=>"invoice_no", "total"=>false, "format"=>"text");
    ...
    //             $header  = array("data"=>$this->useclass->r_header("Purchase Summary Report - ".$this->sd['from']." To ".$this->sd['to']), "style"=>"");
    ...
    //                     $r->quantity = "";
    //                     $r->price = "";
    //                     $r->discount = array("data"=>"Discount : ".$rr->dis,
    //                                      "style"=>"font-weight: bold; border-bottom: 1px solid #000; ");
    //                     $r->cs = "";
    //                     $r->total = "";
    ...
    //                 }
    //                 $tt = ($rr->quantity*$rr->cost)-$rr->discount;
    //                 $t += $tt; $st += $tt;
    //                 $r = new stdClass();
    //                 $r->code = $rr->item_code;
    //                 $r->name = $rr->item_name;
    ...
    //                 $r->discount = number_format($rr->discount, 2, '.', ',');
    //                 $r->cs = number_format($rr->cs, 2, '.', ',');
    ...
    //             $r->discount = "";
    //             $r->cs = "";
    //             $r->total = array("data"=>number_format($t-$rr->dis, 2, '.', ','), "style"=>"border-bottom: 2px solid #000; font-weight: bold; text-align:right;");
    ...
    //             $dis = array("data"=>"Discount", "style"=>"text-align: right; ", "chalign"=>"text-align: right; width:80px");
    //             $cs = array("data"=>"Cost", "style"=>"text-align: right; ", "chalign"=>"text-align: right; width:80px");
    //             $total = array("data"=>"Total", "style"=>"text-align: right;", "chalign"=>"text-align: right; width:100px");
                
    //             $heading = array($name, $code, $qun, $pri,$dis, $total);
    ...
    //             $dis  = array("data"=>"discount", "total"=>false, "format"=>"text");
    //             $cs  = array("data"=>"cs", "total"=>false, "format"=>"text");
    //             $total  = array("data"=>"total", "total"=>false, "format"=>"text");
                
    //             $field = array($name, $code, $qun, $pri,$dis, $total);
                
    //             $page_rec = 28;
                
    //             $header  = array("data"=>$this->useclass->r_header("Purchase Detail Report - ".$this->sd['from']." To ".$this->sd['to']), "style"=>"");
    //             $footer  = array("data"=>"<hr />Report generate on - ".date("Y-m-d H:i:s"), "style"=>"font-size: 11px; font-family: Courier;");
    //             $page_no = array("style"=>"font-size: 11px; font-family: Courier;", "vertical"=>"buttom", "horizontal"=>"right");
