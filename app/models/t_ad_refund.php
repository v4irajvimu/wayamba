...
class t_ad_refund extends CI_Model {
	...
	$this->db->select(array('id','date','ref_no','customer','advance_pay_no','amount','balance','name','cash','cheque','is_cancel'));
...
//        $query=$this->db->query($sqlo);
        ...
        //foreach($query->result() as $r){
            $s .= "<option title='".$a["sum"]->advance_pay_no."' value='".$a["sum"]->advance_pay_no."'>".$a["sum"]->advance_pay_no."</option>";
        //}
        $s .="<select>";
