...
class t_customer_opening_balance extends CI_Model {
	...
	$this->tb_sum = $this->tables->tb['t_cus_opening_bal_sum'];
	$this->tb_det = $this->tables->tb['t_cus_opening_bal_det'];
	...
	if(count($b)){ $this->db->insert_batch($this->tb_acc_trance, $b); }
	 ...
	 $des = "Open Customer Balance";
         
         if($_POST['dr_total']>0)
         {    
         $this->account->set_value($des, $_POST['dr_total'], "dr", "debtor_control");
         $this->account->set_value($des, $_POST['dr_total'], "cr", "sales");
         }
         if($_POST['cr_total']>0)
         {    
         $this->account->set_value($des, $_POST['cr_total'], "cr", "debtor_control");
         $this->account->set_value($des, $_POST['cr_total'], "dr", "sales");
         }
         $this->account->send();
        
	$this->db->trans_complete();
	
	redirect(base_url()."?action=t_customer_opening_balance");
    }
	...
	$this->db->select(array('id', 'no', 'date','is_cancel', 'memo'));
        ...
        if(isset($a["sum"]->id)){
            $this->db->select(array( $this->tb_det.'.customer', $this->tb_det.'.dr', $this->tb_det.'.cr', $this->tb_cus.'.name'));
        $this->db->join($this->tb_cus, $this->tb_cus.".code = ".$this->tb_det.".customer", "INNER");
	...
	$this->db->delete($this->tb_det);
        ...
        $this->load->model('account');
	$this->account->delete($_POST['id'], "OP_CUS", $this->sd['db_code']);
	$this->load->database($this->sd['db'], true);
    }
	...
	$this->db->where('dr_trnce_no', $_POST['id']);
	$this->db->where('module', 'OPEN');
