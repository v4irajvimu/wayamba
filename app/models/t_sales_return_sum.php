...
class t_sales_return_sum extends CI_Model {
  ...
  public function validation2(){
    ...
    $check_item_price_validation=$this->utility->check_item_price('0_',$_POST['inv_no'],$_POST['type'],'2_');
  ...
  public function save(){
    ...
    public function account_update($condition) {
      ...
      if($condition==1){
        ...
        $this->account->set_value2("Sales Return - ". $this->max_no, $_POST['net_amount'], "cr", $_POST['customer'],$condition);

        $srn_amount = $_POST['net_amount']-$_POST['addi_amount'];
        ...
        for ($x = 0; $x < 25; $x++){
          if (isset($_POST['000_' . $x]) && isset($_POST['222_' . $x])){
            if (!empty($_POST['000_' . $x]) && !empty($_POST['222_' . $x])) {
              ...
              $this->db->where('code', $_POST['000_' . $x]);
    ...
    public function load() {
      ...
      $this->db->group_by('t_sales_return_det.code','t_sales_return_det.batch_no');
    //  $this->db->GROUP BY('t_sales_return_det.batch_no');
    ...
    public function PDF_report() {
        ...
        $this->db->select(array('gross_amount', 'net_amount', 'inv_no', 'sales_type','addi_add','addi_deduct'));
        $this->db->where('t_sales_return_sum.cl', $this->sd['cl']);
        $this->db->where('t_sales_return_sum.bc', $this->sd['branch']);
