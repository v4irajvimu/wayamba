...
class r_credit_sales_ret extends CI_Model{
  ...
  return $a;
     ...
     public function PDF_report(){
          ...
          $r_detail['type']="r_credit_sales_ret";          
          ...
          */
          /*$query = $this->db->query('SELECT s.`nno` ,s.`ddate` ,s.`cus_id` ,s.`gross_amount` ,s.`net_amount`  ,c.`name` , s.`rep` ,s.`store` , s.`additional`
            ...
            */
        	...
        	WHERE s.`is_cancel`="0" AND s.`cl`="'.$this->sd['cl'].'" AND s.`bc`= "'.$this->sd['branch'].'" AND s.`sales_type` = "'.$sType.'"
