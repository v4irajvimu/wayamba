...
class r_customer_type extends CI_Model {
    private $sd;
    private $mtb;
    ...
    function __construct(){
    	...
    	$this->mtb = $this->tables->tb['r_customer_type'];
    }
        
    public function base_details(){
        $a['table_data'] = $this->data_table();
        $a['max_no'] = $this->utility->is_auto_genarate("r_customer_type","max_no","is_auto_genarate_cus_type","cus_type_code_type");
    	return $a;
    }
    ...
    public function data_table(){
        ...
        foreach($query->result() as $r){
            ...
            if($this->user_permissions->is_delete('r_customer_type')){$but .= "<img src='".base_url()."img/delete.gif' onclick='set_delete(\"".$r->code."\")' title='Delete' />";}
    ...
    public function save(){
        ...
        try { 
            ...
            if($_POST['code_'] == "0" || $_POST['code_'] == ""){
                if($this->user_permissions->is_add('r_customer_type')){
                    $arr=array(
                        "code"=>$_POST['code'],
                        "description"=>$_POST['description'],
                        "oc"=>$this->sd['oc'],
                        "max_no"=>$this->utility->max_nno("r_customer_type","max_no"),
                    );
                ...
                if($this->user_permissions->is_edit('r_customer_type')){
    ...
    public function delete_validation(){
        $status=1;
        $codes=$_POST['code'];
        $check_cancellation = $this->utility->check_account_trans($codes,'customer type','r_customer_type','m_customer','type');
    ...
    public function delete(){
        ...
        set_error_handler('exceptionThrower'); 
        try {
            if($this->user_permissions->is_delete('r_customer_type')){
