...
class t_price_change_sum extends CI_Model {
    ...
    public function base_details(){
      ...
      $a['max_no'] =$this->get_max_no("t_price_change_sum","nno"); 
	   return $a;
    }
    
    
    public function validation(){
      $this->max_no=$this->get_max_no("t_price_change_sum","nno"); 
      $status=1;
      ...
      try { 
        ...
        for($x = 0; $x<25; $x++){
          ...
          if($_POST['hid'] == "0" || $_POST['hid'] == ""){
            if($this->user_permissions->is_add('t_price_change_sum')){

              $this->db->insert('t_price_change_sum', $a);
              if(count($b)){$this->db->insert_batch("t_price_change_det",$b);}
            ...
            if($this->user_permissions->is_edit('t_price_change_sum')){
              $this->db->where('nno',$_POST['id']);
              $this->db->update('t_price_change_sum', $a);
              $this->set_delete();
              if(count($b)){$this->db->insert_batch("t_price_change_det",$b);}
...
public function get_item_detail(){
    ...
    $sql="SELECT m_item.model,t_price_change_det.*,t_price_change_sum.ddate 
          FROM t_price_change_det JOIN t_price_change_sum ON t_price_change_sum.nno=t_price_change_det.nno  
          JOIN m_item ON m_item.code=t_price_change_det.item WHERE item='$code' AND  t_price_change_sum.ddate BETWEEN '$from' AND '$to' GROUP BY nno LIMIT 25 ";
    ...
    $this->db->select(array(
        't_price_change_sum.nno' ,
        't_price_change_sum.ddate',
        't_price_change_sum.ref_no',
        'm_item.code',
        'm_item.description' ,
        'm_item.model',
        'm_item.purchase_price',
        't_price_change_det.last_price',
        't_price_change_det.last_price_new',
        't_price_change_det.max_price',
        't_price_change_det.max_price_new',
      ));

    $this->db->from('t_price_change_sum');
    $this->db->join('t_price_change_det','t_price_change_sum.nno=t_price_change_det.nno');
    $this->db->join('m_item','m_item.code=t_price_change_det.item');
    $this->db->where('t_price_change_sum.nno',$_POST['id']);
    $this->db->order_by('t_price_change_det.auto_no', 'asc');
    $query=$this->db->get();
    ...
    public function delete(){
      ...
      set_error_handler('exceptionThrower'); 
      try {
        if($this->user_permissions->is_delete('t_price_change_sum')){
          $this->db->where('nno', $_POST['id']);
          $this->db->where('cl', $this->sd['cl']);
          $this->db->where('bc',$this->sd['branch']);
          $this->db->delete("t_quotation_det");
    ...
    public function get_max_no(){
        ...
        $this->db->select_max($field);
       	$this->db->from('t_price_change_sum') ;
        return $this->db->get()->first_row()->$field+1;
    }
    ...
    public function PDF_report(){
     ...
     $query=$this->db->get('t_price_change_sum');
     ...
     $r_detail['orientation']=$_POST['orientation'];
      ...
      $this->db->join('t_price_change_det','t_price_change_sum.nno=t_price_change_det.nno');
      $this->db->join('m_item','m_item.code=t_price_change_det.item');
      $this->db->where('t_price_change_sum.nno',$_POST['qno']);
