...
class r_purchase_return extends CI_Model {
    ...
    public function base_details(){
        ...
        // $a['title'] = "Purchase Return Report";
    ...
    //     if($this->sd['type'] == 'sum'){
    //         $sql = "SELECT
    //                     t_purchse_return_sum.no,
    //                     m_supplier.full_name,
    //                     t_purchse_return_sum.purchase_no,
    //                     m_stores.description,
    //                    (pri.cost)-discount AS cost
    //                   FROM t_purchse_return_sum
    //                     INNER JOIN m_supplier
    //                       ON (m_supplier.code = t_purchse_return_sum.supplier)
    //                     INNER JOIN m_stores
    //                       ON (m_stores.code = t_purchse_return_sum.stores)
    //                     LEFT OUTER JOIN (SELECT
    //                                        SUM(quantity*cost)-t_purchse_return_det.discount AS cost,
    //                                        id
    //                                      FROM t_purchse_return_det
    //                                      GROUP BY id) AS pri
    //                       ON (pri.id = t_purchse_return_sum.id)
    //                   WHERE t_purchse_return_sum.date BETWEEN '".$this->sd['from']."'
    //                       AND '".$this->sd['to']."'
    //                       AND t_purchse_return_sum.is_cancel = 0
    //                       AND bc = '".$this->sd['bc']."'
    //                   ORDER BY t_purchse_return_sum.id";
    //     }else{
    //         $sql = "SELECT
    //                     t_purchse_return_sum.no,
    //                     full_name,
    ...
    //                     quantity,
    //                     t_purchse_return_det.cost,
    //                     t_purchse_return_det.discount
    //                   FROM t_purchse_return_det
    //                     INNER JOIN t_purchse_return_sum
    //                       ON (t_purchse_return_sum.id = t_purchse_return_det.id)
    //                     INNER JOIN m_supplier
    //                       ON (m_supplier.code = t_purchse_return_sum.supplier)
    //                     INNER JOIN m_items
    //                       ON (m_items.code = t_purchse_return_det.item_code)
    //                   WHERE t_purchse_return_sum.date BETWEEN '".$this->sd['from']."'
    //                       AND '".$this->sd['to']."'
    //                       AND t_purchse_return_sum.is_cancel = 0
    //                       AND bc = '".$this->sd['bc']."'
    ...
    //             $vehi = array("data"=>"Purchase No","style"=>"text-align: left; ", "chalign"=>"text-align: left; width:100px");
    ...
    //             $vehi  = array("data"=>"purchase_no", "total"=>false, "format"=>"text");
    ...
    //             $header  = array("data"=>$this->useclass->r_header("Purchase Return Summary Report - ".$this->sd['from']." To ".$this->sd['to']), "style"=>"");
