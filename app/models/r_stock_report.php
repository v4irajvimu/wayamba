...
class r_stock_report extends CI_Model {
    ...
    public function get_default_store(){
        ...
        FROM
          m_branch mb 
           WHERE mb.cl ='".$this->sd['cl']."' AND mb.bc = '".$this->sd['branch']."'";
          $s = $this->db->query($sql)->first_row();
  ...
  public function get_stores(){
	...
	public function PDF_report(){
		...
		$sql="SELECT m_item.`code`, m_item.`description`,m_item.`model`,qry_current_stock.`qty` FROM m_item
		JOIN qry_current_stock ON qry_current_stock.`item`=m_item.`code` WHERE qry_current_stock.`store_code`='$_POST[stores]'
