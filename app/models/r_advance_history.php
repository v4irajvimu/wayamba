...
class r_advance_history extends CI_Model {
 ...
 //        $sql = "SELECT 
 //            `t_advance_pay_trance`.`date` AS `date`,
 //            `t_advance_pay_trance`.`cr_trance_code` AS cr_trnce_code,
 //            `t_advance_pay_trance`.`cr_trance_no` AS cr_trnce_no,
 //            `t_advance_pay_trance`.`dr_trance_no` AS dr_trnce_no,
 //            `t_advance_pay_trance`.`dr_trance_code` AS dr_trnce_code,
 //            `t_advance_pay_trance`.`dr_trance_amount` AS dr_amount,
 //            `t_advance_pay_trance`.`cr_trance_amount` AS cr_amount, 
 //             IFNULL(b.adv_bal,0) AS opbal 

 //          FROM
 //            `t_advance_pay_trance` 
 //            INNER JOIN `m_customer` 
 //              ON (
 //                `t_advance_pay_trance`.`customer` = `m_customer`.`code`
 //              ) 

 //             CROSS JOIN(SELECT (SUM(`dr_trance_amount`)-SUM(`cr_trance_amount`))
 //                                        AS adv_bal FROM `t_advance_pay_trance`
 //                                        WHERE `customer` = '".$this->sd['customer']."'  AND `date` < '".$this->sd['from']."' )AS b

 //          WHERE `t_advance_pay_trance`.`customer` = '".$this->sd['customer']."'  
 //            AND `t_advance_pay_trance`.`date` BETWEEN '".$this->sd['from']."' 
 //                                AND '".$this->sd['to']."' 
 //          GROUP BY `t_advance_pay_trance`.`customer`,
 //            `t_advance_pay_trance`.`dr_trance_code`,
 //            `t_advance_pay_trance`.`dr_trance_no` 
 //          ORDER BY `t_advance_pay_trance`.`date`,
 //            `t_advance_pay_trance`.`cr_trance_no` ";
 ...
 //                $header  = array("data"=>$this->useclass->r_header("Advance Settlement Report:- ".$this->sd['from']." To ".$this->sd['to'].'~~'.$que->outlet_name." (".$que->code.")"), "style"=>"");
