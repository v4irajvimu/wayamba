...
class r_hp_total_outstanding extends CI_Model{
  ...
  public function PDF_report(){
    ...
    s.agreement_no,s.cus_id,mc.`nic`,mc.`name`,d.`item_code`,s.`net_amount`,IFNULL(c.panalty,0) AS panalty,IFNULL(c1.other_c,0) AS othr_charges ,IFNULL(c2.tot_paid,0) AS total_paid, IFNULL(s.`net_amount`,0) -IFNULL(c2.tot_paid, 0)  AS bal  FROM `t_hp_sales_sum` s
    ...
    WHERE s.cl='".$this->sd['cl']."' AND s.bc='".$this->sd['branch']."'AND s.`ddate` <='".$_POST['to']."' ";
    ...
    $sql.="GROUP BY s.agreement_no ORDER BY s.nno";
