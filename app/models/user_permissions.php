...
class user_permissions extends CI_Model {
    ...
    function is_approve($mod){
       ...
       // echo $sql; 
        $query = $this->db->query($sql);
        if($query->num_rows){
            return $query->first_row()->is_approve;
        }else{
            return 0;
        }
       // return 1;
   ...
   echo $query->first_row()->is_delete;
    ...
    public function view_module($colum){
    ...
    public function view_mod_permission($module){
     
      $sql="SELECT is_active FROM `def_option_module` WHERE module_name ='$module'";

      $query = $this->db->query($sql);
      
      if($query->num_rows()>0){

        $querys = $query->row()->is_active;
