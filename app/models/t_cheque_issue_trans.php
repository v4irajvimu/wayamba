...
class t_cheque_issue_trans extends CI_Model {
                ...
                for ($i = 0; $i < 25; $i++) {
                  if(isset($_POST['5_'.$i] , $_POST['0_'.$i], $_POST['4_'.$i])){
                    if ($_POST['0_' . $i] != "" && $_POST['0_' . $i] != "0") {                   
                      ...
                      $sql="UPDATE t_cheque_book_det d
                      JOIN t_cheque_book_sum s ON s.`cl` = d.`cl` AND s.`bc` = d.`bc` AND s.`nno` = d.`nno`
                      SET d.`status` ='3'
                      WHERE d.cl='".$this->sd['cl']."'
                      AND d.bc ='".$this->sd['branch']."'
                      AND d.`cheque_no`='".$_POST['2_'.$i]."'
                      AND s.`account`='".$_POST['0_'.$i]."'";
    ...
    try {  
      if($this->user_permissions->is_delete('t_cheque_issue_trans')){
        ...
        foreach ($this->db->query($sql)->result() as $row) {
          ...
          WHERE d.cl='".$this->sd['cl']."'
          AND d.bc ='".$this->sd['branch']."'
          AND d.`cheque_no`='".$row->cheque."'
          AND s.`account`='".$row->account."'";
  ...
  public function load_cheques(){
    ...
    */
          ...
          $sql.=" GROUP BY cheque_no,account";
