...
class r_sup_category extends CI_Model {
	...
	$this->mtb = $this->tables->tb['r_sup_category'];
    $this->load->model('user_permissions');
    }
    
    public function base_details(){
	$a['table_data'] = $this->data_table();
	$a['max_no'] = $this->utility->is_auto_genarate("r_sup_category","max_no","is_auto_genarate_sup_cat","sup_cat_code_type");
	return $a;
    }
    ...
    public function data_table(){
       ...
       $this->db->limit(10);
        ...
        foreach($query->result() as $r){
            ...
            if($this->user_permissions->is_delete('r_sup_category')){$but .= "<img src='".base_url()."img/delete.gif' onclick='set_delete(\"".$r->code."\")' title='Delete' />";}
    ...
    public function save(){
        ...
        try {
            ...
            if($_POST['code_'] == "0" || $_POST['code_'] == ""){
                if($this->user_permissions->is_add('r_sup_category')){
            		$arr=array(
                        "code"=>$_POST['code'],
                        "description"=>$_POST['description'],
                        "oc"=>$this->sd['oc'],
                        "max_no"=>$this->utility->max_nno("r_sup_category","max_no"),
                    );
                ...
                if($this->user_permissions->is_edit('r_sup_category')){
    ...
    public function delete_validation(){
        $status=1;
        $codes=$_POST['code'];
        $check_cancellation = $this->utility->check_account_trans($codes,'supplier category','r_sup_category','m_supplier','category');
    ...
    public function delete(){
        ...
        set_error_handler('exceptionThrower'); 
        try { 
            if($this->user_permissions->is_delete('r_sup_category')){
