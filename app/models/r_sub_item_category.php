...
class r_sub_item_category extends CI_Model {
...
///////
    ...
    if(!empty($cluster))
    {
       $sql="SELECT c.`code`,
                c.`description`,
                s.`code` AS SubCat,
                s.`description` AS SubCatName 
          FROM  `r_category` c 
          LEFT JOIN `r_sub_category` s ON c.`code` = s.`main_category` 
          LEFT JOIN m_item m ON m.`category`= s.`code`
          INNER JOIN qry_current_stock i ON i.`item` = m.`code`
          WHERE m.`code` != '' ";
    }
    else
    {
