...
class t_supplier_settlement extends CI_Model {
	...
	$this->tb_sum = $this->tables->tb['t_supplier_settle_sum'];
	$this->tb_det = $this->tables->tb['t_supplier_settle_det'];
	...
	redirect(base_url()."?action=t_supplier_settlement");//&print=.$lid
    }
    
    public function load(){
        $this->db->where("no", $_POST['id']);
        $this->db->where("bc", $this->sd['bc']);
	$this->db->join($this->tb_cus, $this->tb_cus.".code = ".$this->tb_sum.".supplier", "INNER");
        $this->db->limit(1);
