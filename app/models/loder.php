...
class loder extends CI_Model {
    ...
    function __construct(){
        ...
        $this->user_role=$this->tables->tb['u_user_role'];
    ...
    public function default_settings(){
    	...
    	foreach($query->result() as $row){
    		...
    		$a['salesman_cat_code']=$row->def_salesman_code;
    		$a['use_collection_officer']=$row->use_collection_officer;
    		$a['collection_officer_cat_code']=$row->def_collection_officer_code;
    	}
    ...
    public function select_user_role(){
        $query = $this->db->get($this->user_role);
        $s = "<select name='user_role' id='user_role' style='width:185px;'>";
        $s .= "<option value='0'>---</option>";
        foreach($query->result() as $r){
        $s .= "<option title='".$r->role_id."' value='".$r->role_id."'>".$r->role_id." - ".$r->description."</option>";
        }
        $s .= "</select>";
        return $s;
    }
    ...
    public function branch_det(){
    	...
    	return "(".$this->sd['cl'].") ".$cl." | (".$this->sd['branch'].") ".$bc;
    }
