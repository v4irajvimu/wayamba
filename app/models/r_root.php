...
class r_root extends CI_Model {
	...
	$this->mtb = $this->tables->tb['r_root'];
	
    }
    
    public function base_details(){
	$a['table_data'] = $this->data_table();
    $a['max_no'] = $this->utility->is_auto_genarate("r_root","max_no","is_auto_genarate_root","root_code_type");
	return $a;
    }
    ...
    public function data_table(){
        ...
        //$this->db->from('r_root') --- Assign database separatly;
        ...
        foreach($query->result() as $r){
            ...
            if($this->user_permissions->is_delete('r_root')){$but .= "<img src='".base_url()."img/delete.gif' onclick='set_delete(\"".$r->code."\")' title='Delete' />";}
    ...
    public function save(){
        ...
        try { 
        	...
        	{
        		if($_POST['code']=="0" || $_POST['code_'] == "")
        		{
                    if($this->user_permissions->is_add('r_root')){
            			$arr=array(
                            "code"=>$_POST['code'],
                            "description"=>$_POST['description'],
                            "oc"=>$this->sd['oc'],
                            "max_no"=>$this->utility->max_nno("r_root","max_no"),
                        );
        		...
        		{
                    if($this->user_permissions->is_edit('r_root')){
    ...
    public function delete_validation(){
        $status=1;
        $codes=$_POST['code'];
        $check_cancellation = $this->utility->check_account_trans($codes,'root','r_root','m_customer','root');
    ...
    public function delete(){
        ...
        set_error_handler('exceptionThrower'); 
        try {
            if($this->user_permissions->is_delete('r_root')){
    ...
    public function select($type='code', $style=''){
    	...
    	$s = "<select name='root' id='root' style='".$style."'>";
