...
class r_serial_in_hand extends CI_Model {
...
///////
		...
		if($cluster == '0'){
			...
			$sql="SELECT
				...
				INNER JOIN (SELECT item,SUM(qty_in-qty_out) AS item_tot FROM t_item_movement WHERE ddate BETWEEN '".$from."' AND '".$to."' GROUP BY item ) c ON t_serial.item = c.item 
				INNER JOIN t_serial_movement ON t_serial.`item`=t_serial_movement.`item` AND t_serial.`serial_no`=t_serial_movement.`serial_no`
				AND t_serial.`store_code` = t_serial_movement.store_code
				WHERE t_serial.`date` 
				BETWEEN '".$from."' AND '".$to."' AND t_serial.`available`='1'
				";
		}
		...
		if($cluster != '0'){
			...
			$sql="SELECT 
				...
				FROM t_serial
				JOIN m_item ON t_serial.`item`=m_item.`code`
				INNER JOIN (SELECT item,SUM(qty_in-qty_out) AS item_tot FROM t_item_movement WHERE ddate BETWEEN '".$from."' AND '".$to."' AND t_item_movement.cl='".$cluster."' GROUP BY item ) c ON t_serial.item = c.item 
				INNER JOIN t_serial_movement ON t_serial.`item`=t_serial_movement.`item` AND t_serial.`serial_no`=t_serial_movement.`serial_no`
				AND t_serial.`store_code` = t_serial_movement.store_code
				WHERE t_serial.`date` 
				BETWEEN '".$from."' AND '".$to."' AND t_serial.`available`='1'
		...
		if($cluster!='0' && $branch !='0'){
			...
			$sql="SELECT
				...
				FROM t_serial
				JOIN m_item ON t_serial.`item`=m_item.`code`
				INNER JOIN (SELECT item,SUM(qty_in-qty_out) AS item_tot FROM t_item_movement WHERE ddate BETWEEN '".$from."' AND '".$to."' AND t_item_movement.cl='".$cluster."' AND t_item_movement.bc='".$branch."' GROUP BY item ) c ON t_serial.item = c.item 
				INNER JOIN t_serial_movement ON t_serial.`item`=t_serial_movement.`item` AND t_serial.`serial_no`=t_serial_movement.`serial_no`
				AND t_serial.`store_code` = t_serial_movement.store_code
				WHERE t_serial.`date` 
				BETWEEN '".$from."' AND '".$to."' AND t_serial.`available`='1'
		...
		if($cluster!='0' && $branch=='0' && $store!='0'){
			$sql="SELECT 
				...
				FROM t_serial
				JOIN m_item ON t_serial.`item`=m_item.`code`
				INNER JOIN (SELECT item,SUM(qty_in-qty_out) AS item_tot FROM t_item_movement WHERE ddate BETWEEN '".$from."' AND '".$to."' AND t_item_movement.cl='".$cluster."' AND t_item_movement.store_code='".$store."' GROUP BY item ) c ON t_serial.item = c.item 
				INNER JOIN t_serial_movement ON t_serial.`item`=t_serial_movement.`item` AND t_serial.`serial_no`=t_serial_movement.`serial_no`
				AND t_serial.`store_code` = t_serial_movement.store_code
				WHERE t_serial.`date` 
				BETWEEN '".$from."' AND '".$to."' AND t_serial.`available`='1'
		...
		if($cluster !='0' && $branch !='0' && $store !='0'){
			$sql="SELECT
				...
				FROM t_serial
				JOIN m_item ON t_serial.`item`=m_item.`code`
				INNER JOIN (SELECT item,SUM(qty_in-qty_out) AS item_tot FROM t_item_movement WHERE ddate BETWEEN '".$from."' AND '".$to."' AND t_item_movement.cl='".$cluster."' AND t_item_movement.bc='".$branch."' AND t_item_movement.store_code='".$store."' GROUP BY item ) c ON t_serial.item = c.item 
				INNER JOIN t_serial_movement ON t_serial.`item`=t_serial_movement.`item` AND t_serial.`serial_no`=t_serial_movement.`serial_no`
				AND t_serial.`store_code` = t_serial_movement.store_code
				WHERE t_serial.`date` 
				BETWEEN '".$from."' AND '".$to."' AND t_serial.`available`='1'
		...
		// 	FROM t_serial
		// 	JOIN m_item ON t_serial.`item`=m_item.`code`
		// 	INNER JOIN (SELECT item,SUM(qty_in-qty_out) AS item_tot FROM t_item_movement WHERE ddate BETWEEN '".$from."' AND '".$to."' AND t_item_movement.store_code='".$store."' GROUP BY item ) c ON t_serial.item = c.item 
		// 	WHERE t_serial.`date` 
		// 	BETWEEN '".$from."' AND '".$to."' AND t_serial.`store_code`='".$store."' AND t_serial.`available`='1'
		// 	";
		...
		if(!empty($cluster))
		{
			$sql.=" AND t_serial.`cl` = '".$cluster."'";
		}
		if(!empty($branch))
		{
			$sql.=" AND t_serial.`bc` = '".$branch."'";
		}

		if(!empty($store))
		{
			$sql.=" AND t_serial.`store_code` = '".$store."'";
		}
		...
		$sql.="GROUP BY t_serial_movement.`serial_no`";
