...
class r_color extends CI_Model {
	...
	$a['max_no'] = $this->utility->is_auto_genarate("r_color","max_no","is_auto_genarate_color","color_code_type");
	return $a;
    }
    ...
    public function data_table(){
        ...
        $this->db->limit(10);
        $query = $this->db->get('r_color');
        
        foreach($query->result() as $r){
            $but = "<img src='".base_url()."img/edit.gif' onclick='set_edit(\"".$r->code."\")' title='Edit' />&nbsp;&nbsp;";
            if($this->user_permissions->is_add('r_color')){$but .= "<img src='".base_url()."img/delete.gif' onclick='set_delete(\"".$r->code."\")' title='Delete' />";}
    ...
    public function save(){
        ...
        try {  
            ...
            if($_POST['code_'] == "0" || $_POST['code_'] == ""){
                if($this->user_permissions->is_add('r_color')){
                    $arr=array(
                        "code"=>$_POST['code'],
                        "description"=>$_POST['description'],
                        "code_gen"=>$_POST['code_gen'],
                        "oc"=>$this->sd['oc'],
                        "max_no"=>$this->utility->max_nno("r_color","max_no"),
                    );
                    $this->db->insert('r_color', $arr);
                ...
                if($this->user_permissions->is_edit('r_color')){
                    ...
                    $this->db->update('r_color', $arr_update);
                    echo $this->db->trans_commit();
                }else{
	...
	echo $this->db->get('r_color')->num_rows;
    }
    
    public function load(){
	$this->db->where('code', $_POST['code']);
	$this->db->limit(1);
	echo json_encode($this->db->get('r_color')->first_row());
    }
    ...
    public function delete_validation(){
        $status=1;
        $codes=$_POST['code'];
        $check_cancellation = $this->utility->check_account_trans($codes,'Color','r_color','m_item','color');
    ...
    public function select(){
     ...
     public function auto_com(){
        ...
        $this->db->or_like('r_color'.'.description', $_GET['q']);
        $query = $this->db->select(array('code', 'code_gen', 'r_color'.'.description'))->get('r_color');
        $abc = "";
            foreach($query->result() as $r){
