...
class r_dispatch_status extends CI_Model {
  ...
  //       $sql = "SELECT
  //               `t_dispatch_trance`.`item_code`
  //               , `m_items`.`description`
  //               , `t_dispatch_trance`.`in_no`
  //               , SUM(`t_dispatch_trance`.`in_qty`) AS in_qty
  //               , SUM(`t_dispatch_trance`.`out_qty`) AS out_qty
  //               , `t_dispatch_trance`.`trance_code`
  //               , t_dispatch_trance.`date`
  //           FROM
  //               `t_dispatch_trance`
  //               INNER JOIN `m_items` 
  //                   ON (`t_dispatch_trance`.`item_code` = `m_items`.`code`)
  //                   WHERE `date` BETWEEN '".$this->sd['from']."' AND '".$this->sd['to']."'
  //                   GROUP BY item_code,`in_no`,`trance_code`";
  ...
  //               foreach($result as $r){
		...
		// if ($no != $r->in_no && $trance!=$r->trance_code) {
  ...
  //               $heading = array($no,$date,$code,$name,$in_qty,$out_qty);
  ...
  //               $field = array($no,$date,$code,$name,$in_qty,$out_qty);
  ...
  //           $header  = array("data"=>$this->useclass->r_header("Dispatch Status Report  - ".$this->sd['from']." To ".$this->sd['to']), "style"=>"");
