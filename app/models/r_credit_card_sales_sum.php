...
class r_credit_card_sales_sum extends CI_Model{
  ...
  public function PDF_report(){
    ...
    if($_POST['sales_category']!="0"){
      ...
      $query = $this->db->query('SELECT s.`nno`,
                    ...
                    s.`pay_cash`,
                    ms.description,
                    creditC.`amount` AS amount,
                    creditC.`card_no` AS card_no
                    FROM `t_cash_sales_sum` s
                    JOIN `m_customer` c ON s.`cus_id`=c.`code`
                    JOIN `m_stores` ms ON ms.`code` = s.`store`
                    JOIN `opt_credit_card_det` creditC 
                    ON creditC.`trans_no` = s.`nno` 
                    AND creditC.`cl` =s.`cl` 
                    AND creditC.`bc` =s.`bc` 
                    AND creditC.`trans_code` =s.`type`  
