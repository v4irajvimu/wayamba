...
class t_cash_and_card_sales_sum extends CI_Model {
    ...
    function __construct() {
        ...
        $this->mtb = $this->tables->tb['t_cash_and_card_sales_sum'];
    ...
    public function base_details() {
        ...
        $a['max_no'] = $this->utility->get_max_no("t_cash_and_card_sales_sum", "nno");
   ...
   public function get_credit_max_no() {
    ...
    public function validation() {
        $this->max_no = $this->utility->get_max_no("t_cash_and_card_sales_sum", "nno");
        $status = 1;
        if (empty($_POST['save_status']) && $_POST['save_status'] != "1") {
            return "Please check the payment option";
        }
        $check_is_delete = $this->validation->check_is_cancel($this->max_no, 't_cash_and_card_sales_sum');
        if ($check_is_delete != 1) {
            return "This cash and card sale already deleted";
        }
        ...
        try {
            ...
            if ($validation_status == 1) {
                ...
                for ($x = 0; $x < 25; $x++) {
                    if (isset($_POST['00_' . $x], $_POST['11_' . $x], $_POST['tt_' . $x])) {
                        if ($_POST['00_' . $x] != "" && $_POST['11_' . $x] != "" && $_POST['tt_' . $x] != "") {
                            $t_cash_and_card_sales_additional_item[] = array(
                                "cl" => $this->sd['cl'],
                                "bc" => $this->sd['branch'],
                                "nno" => $this->max_no,
                                "type" => $_POST['00_' . $x],
                ...
                for ($x = 0; $x < 25; $x++) {
                    if (isset($_POST['0_' . $x], $_POST['5_' . $x], $_POST['free_price_' . $x])) {
                        if ($_POST['0_' . $x] != "" && $_POST['5_' . $x] != "" && $_POST['free_price_' . $x] != "") {
                            ...
                            $t_cash_and_card_sales_det[] = array(
                                "cl" => $this->sd['cl'],
                                "bc" => $this->sd['branch'],
                                "nno" => $this->max_no,
                                "code" => $_POST['0_'.$x],
                  ...
                  $t_cash_and_card_sales_sum = array(
                    "cl" => $this->sd['cl'],
                    "bc" => $this->sd['branch'],
                    "oc" => $this->sd['oc'],
                    "nno" => $this->max_no,
                    ...
                    "sub_no" => $this->utility->get_max_sales_category2('sub_no','t_cash_and_card_sales_sum',$_POST['sales_category']),
                    "memo" => $_POST['memo'],
                    "store" => $_POST['stores'],
                ...
                $t_credit_note = array(
                          ...
                          "ddate" => $_POST['date'],
                          "ref_no" => $_POST['ref_no'],
                          "memo" => "FREE ISSUE ITEM - CASH_AND_CARD_SALES - [" . $this->max_no . "]",
                          "is_customer" => 1,
                          "code" => $_POST['customer'],
                ...
                if ($_POST['hid'] == "0" || $_POST['hid'] == "") {
                    if($this->user_permissions->is_add('t_cash_and_card_sales_sum')){
                        $account_update=$this->account_update(0);
                        if($account_update==1){
                        ...
                        if($_POST['approve_status']=="1"){
                            if($total!=0){
                                $this->trans_settlement->save_settlement("t_credit_note_trans", $_POST['customer'], $_POST['date'], 17, $this->credit_max_no, 69, $this->max_no, $total, "0");
                            ...
                            $this->utility->save_logger("APPROVE",69,$this->max_no,$this->mod);
                        ...
                        $this->db->insert($this->mtb, $t_cash_and_card_sales_sum);
                        if (isset($t_cash_and_card_sales_det)) {
                            if (count($t_cash_and_card_sales_det)) {
                                $this->db->insert_batch("t_cash_and_card_sales_det", $t_cash_and_card_sales_det);
                            }
                        }

                        if (isset($t_cash_and_card_sales_additional_item)) {
                            if (count($t_cash_and_card_sales_additional_item)) {
                                $this->db->insert_batch("t_cash_and_card_sales_additional_item", $t_cash_and_card_sales_additional_item);
                            }
                        }

                        $this->load->model('t_payment_option');
                        $this->t_payment_option->save_payment_option($this->max_no, 69);
                    ...
                    if($this->user_permissions->is_edit('t_cash_and_card_sales_sum')){
                        
                        $status=$this->trans_cancellation->cash_sales_update_status($this->max_no,69);  
                        if($status=="OK"){
                            $account_update=$this->account_update(0);
                            if($account_update==1){
                            $this->set_delete();
                            ...
                            if($_POST['approve_status']=="1"){
                                ...
                                $this->trans_settlement->delete_settlement_sub("t_credit_note_trans","69",$this->max_no);   
                                $this->trans_settlement->delete_settlement_sub("t_debit_note_trans","69",$this->max_no); 
                            ...
                            $this->t_payment_option->delete_all_payments_opt(69, $this->max_no);
                            $this->t_payment_option->save_payment_option($this->max_no, 69);
                            ...
                            $this->db->where("cl", $this->sd['cl']);
                            $this->db->where("bc", $this->sd['branch']);
                            $this->db->update($this->mtb, $t_cash_and_card_sales_sum);
                            ...
                            $this->utility->save_logger("EDIT",69,$this->max_no,$this->mod);
                            echo $this->db->trans_commit();
                            }else{
                              echo "Invalid account entries";
                              $this->db->trans_commit();
    ...
    private function set_delete() {
        ...
        $this->db->where("cl", $this->sd['cl']);
        $this->db->where("bc", $this->sd['branch']);
        $this->db->delete("t_cash_and_card_sales_det");

        $this->db->where("nno", $this->max_no);
        $this->db->where("cl", $this->sd['cl']);
        $this->db->where("bc", $this->sd['branch']);
        $this->db->delete("t_cash_and_card_sales_additional_item");

        $this->load->model('trans_settlement');
        $this->trans_settlement->delete_item_movement('t_item_movement',69,$_POST['hid']);
    }
    ...
    public function account_update($condition) {
        ...
        if($condition==0){
             $query = $this->db->query("
                 ...
                 WHERE  t.`cl`='" . $this->sd['cl'] . "'  AND t.`bc`='" . $this->sd['branch'] . "'  AND t.`trans_code`='69'  AND t.`trans_no` ='" . $this->max_no . "' AND 
                 a.`is_control_acc`='0'");
    ...
    public function load() {
        $this->db->select(array(
            't_cash_and_card_sales_sum.cl',
            't_cash_and_card_sales_sum.bc',
            't_cash_and_card_sales_sum.nno',
            't_cash_and_card_sales_sum.type',
            't_cash_and_card_sales_sum.ddate',
            't_cash_and_card_sales_sum.ref_no',
            't_cash_and_card_sales_sum.cus_id',
            't_cash_and_card_sales_sum.so_no',
            't_cash_and_card_sales_sum.category',
            't_cash_and_card_sales_sum.sub_no',
            't_cash_and_card_sales_sum.memo',
            't_cash_and_card_sales_sum.store',
            't_cash_and_card_sales_sum.quotation',
            't_cash_and_card_sales_sum.rep',
            't_cash_and_card_sales_sum.gross_amount',
            't_cash_and_card_sales_sum.group_no',
            't_cash_and_card_sales_sum.additional',
            't_cash_and_card_sales_sum.net_amount',
            't_cash_and_card_sales_sum.oc',
            't_cash_and_card_sales_sum.action_date',
            't_cash_and_card_sales_sum.post',
            't_cash_and_card_sales_sum.post_by',
            't_cash_and_card_sales_sum.previlliage_card_no',
            't_cash_and_card_sales_sum.previlliage_point_add',
            't_cash_and_card_sales_sum.pay_cash',
            't_cash_and_card_sales_sum.pay_issue_chq',
            't_cash_and_card_sales_sum.pay_receive_chq',
            't_cash_and_card_sales_sum.pay_ccard',
            't_cash_and_card_sales_sum.pay_cnote',
            't_cash_and_card_sales_sum.pay_dnote',
            't_cash_and_card_sales_sum.pay_discount',
            't_cash_and_card_sales_sum.pay_bank_debit',
            't_cash_and_card_sales_sum.pay_advance',
            't_cash_and_card_sales_sum.pay_gift_voucher',
            't_cash_and_card_sales_sum.pay_credit',
            't_cash_and_card_sales_sum.pay_privi_card',
            't_cash_and_card_sales_sum.is_multi_payment',
            't_cash_and_card_sales_sum.is_cancel',
            't_cash_and_card_sales_sum.crn_no',
            't_cash_and_card_sales_sum.is_bulk_foc',
            't_cash_and_card_sales_sum.cus_name',
            't_cash_and_card_sales_sum.cus_address',
            't_cash_and_card_sales_sum.additional_add',
            't_cash_and_card_sales_sum.additional_deduct',
            'm_customer.name',
            'm_customer.address1',
            'm_customer.address2',
            'm_customer.address3',
        ...
        $this->db->from('t_cash_and_card_sales_sum');
        $this->db->join('m_customer', 'm_customer.code=t_cash_and_card_sales_sum.cus_id');
        $this->db->join('m_employee', 'm_employee.code=t_cash_and_card_sales_sum.rep');

        $this->db->where('t_cash_and_card_sales_sum.cl', $this->sd['cl']);
        $this->db->where('t_cash_and_card_sales_sum.bc', $this->sd['branch']);
        $this->db->where('t_cash_and_card_sales_sum.nno', $_POST['id']);
        ...
        $this->db->select(array(
            't_cash_and_card_sales_det.code',
            't_cash_and_card_sales_det.qty',
            't_cash_and_card_sales_det.discountp',
            't_cash_and_card_sales_det.discount',
            't_cash_and_card_sales_det.price',
            't_cash_and_card_sales_det.amount',
            't_cash_and_card_sales_det.cost',
            't_cash_and_card_sales_det.foc',
            't_cash_and_card_sales_det.batch_no',
            't_cash_and_card_sales_det.warranty',
            't_cash_and_card_sales_det.is_free',
            't_cash_and_card_sales_det.free_balance',
            'm_item.description as item_des',
            'm_item.model',
            't_item_batch.min_price'
        ));

        $this->db->from('t_cash_and_card_sales_det');
        $this->db->join('m_item', 'm_item.code=t_cash_and_card_sales_det.code');
        $this->db->join('t_item_batch', 't_item_batch.item = t_cash_and_card_sales_det.code AND  t_item_batch.batch_no= t_cash_and_card_sales_det.batch_no');
        $this->db->where('t_cash_and_card_sales_det.cl', $this->sd['cl']);
        $this->db->where('t_cash_and_card_sales_det.bc', $this->sd['branch']);
        $this->db->where('t_cash_and_card_sales_det.nno', $_POST['id']);
        $this->db->order_by('t_cash_and_card_sales_det.auto_num', "asc");
        $query = $this->db->get();
        ...
        $this->db->select(array(
            't_cash_and_card_sales_additional_item.type as sales_type',
            't_cash_and_card_sales_additional_item.rate_p',
            't_cash_and_card_sales_additional_item.amount',
            'r_additional_item.description'
        ));

        $this->db->from('t_cash_and_card_sales_additional_item');
        $this->db->join('r_additional_item', 'r_additional_item.code=t_cash_and_card_sales_additional_item.type');
        $this->db->where('t_cash_and_card_sales_additional_item.cl', $this->sd['cl']);
        $this->db->where('t_cash_and_card_sales_additional_item.bc', $this->sd['branch']);
        $this->db->where('t_cash_and_card_sales_additional_item.nno', $_POST['id']);
        $query = $this->db->get();
        ...
        $this->db->join('t_cash_and_card_sales_sum', 't_serial.out_no=t_cash_and_card_sales_sum.nno');
        $this->db->where('t_serial.out_doc', 69);
    ...
    public function delete() {
        ...
        set_error_handler('exceptionThrower'); 
        try {
            if($this->user_permissions->is_delete('t_cash_and_card_sales_sum')){
                $bc=$this->sd['branch'];
                $cl=$this->sd['cl'];
                $trans_no=$_POST['trans_no'];
                $status=$this->trans_cancellation->cash_sales_update_status('no',69);  

                if($status=="OK"){

                    $this->load->model('trans_settlement');
                    $this->trans_settlement->delete_item_movement('t_item_movement',69,$trans_no);
                    ...
                    $this->load->model('trans_settlement');
                    $this->trans_settlement->delete_settlement_sub("t_credit_note_trans","69",$trans_no);   
                    $this->trans_settlement->delete_settlement_sub("t_debit_note_trans","69",$trans_no); 
                  ...
                  $this->db->update('t_cash_and_card_sales_sum',$data);

                  $sql="SELECT cus_id FROM t_cash_and_card_sales_sum WHERE cl='".$this->sd['cl']."' AND bc='".$this->sd['branch']."' AND nno='".$_POST['trans_no']."'";
                  ...
                  $this->utility->save_logger("CANCEL",69,$_POST['trans_no'],$this->mod);
                  echo $this->db->trans_commit();
                }else{
                    $this->db->trans_commit();
      ...
      else
      {
        $sql="SELECT SUM(qry_current_stock.`qty`)+SUM(c.qty) as qty 
                ...
                INNER JOIN (SELECT qty,code,batch_no,cl,bc 
                            FROM t_cash_and_card_sales_det 
    ...
    public function PDF_report() {
        ...
        $r_detail['r_type'] = "CASH AND CARD SALES";
        ...
        $sql_query="SELECT ai.is_add AS is_add, SUM( IF(ai.`is_add`,1,-1) *ca.`amount`) AS amt
                    FROM `t_cash_and_card_sales_additional_item` ca
        ...
        $sql="SELECT `t_cash_and_card_sales_det`.`code`, 
                    `t_cash_and_card_sales_det`.`qty`, 
                    `t_cash_and_card_sales_det`.`discount`, 
                    `t_cash_and_card_sales_det`.`price`, 
                    `t_cash_and_card_sales_det`.`amount`, 
                    ...
                    c.`cc` AS sub_item,
                    c.`deee` AS des,
                    c.qty *  t_cash_and_card_sales_det.qty as sub_qty
            FROM (`t_cash_and_card_sales_det`) 
            JOIN `m_item` ON `m_item`.`code` = `t_cash_and_card_sales_det`.`code` 
            LEFT JOIN `m_item_sub` ON `m_item_sub`.`item_code` = `m_item`.`code` 
            LEFT JOIN `r_sub_item` ON `r_sub_item`.`code` = `m_item_sub`.`sub_item` 
            LEFT JOIN (
                        SELECT t_cash_and_card_sales_det.`code`, 
                        ...
                        LEFT JOIN t_cash_and_card_sales_det ON t_cash_and_card_sales_det.`code` = t_item_movement_sub.`item`  
                        AND t_cash_and_card_sales_det.`cl` = t_item_movement_sub.`cl` AND t_cash_and_card_sales_det.`bc`=t_item_movement_sub.`bc`
                        JOIN r_sub_item ON t_item_movement_sub.`sub_item` = r_sub_item.`code`
                        WHERE t_item_movement_sub.batch_no = t_cash_and_card_sales_det.`batch_no` AND `t_cash_and_card_sales_det`.`cl` = '".$this->sd['cl']."'  
                        AND `t_cash_and_card_sales_det`.`bc` = '".$this->sd['branch']."' AND `t_cash_and_card_sales_det`.`nno` = '".$_POST['qno']."'  )c ON c.code = t_cash_and_card_sales_det.`code`
            WHERE `t_cash_and_card_sales_det`.`cl` = '".$this->sd['cl']."' 
            AND `t_cash_and_card_sales_det`.`bc` = '".$this->sd['branch']."'
            AND `t_cash_and_card_sales_det`.`nno` = '".$_POST['qno']."'  
            GROUP BY c.cc ,t_cash_and_card_sales_det.`code`
            ORDER BY `t_cash_and_card_sales_det`.`auto_num` ASC 
        ";

        $query = $this->db->query($sql);
        $r_detail['items'] = $this->db->query($sql)->result();

        $ssql=" SELECT card_no, ddate, opt_credit_card_det.amount,card_type,expire_date   
                FROM opt_credit_card_det
                JOIN t_cash_and_card_sales_sum ON t_cash_and_card_sales_sum.nno = opt_credit_card_det.`trans_no` 
                AND t_cash_and_card_sales_sum.cl = opt_credit_card_det.cl
                AND t_cash_and_card_sales_sum.bc = opt_credit_card_det.bc
                WHERE trans_code = '69' AND trans_no = '".$_POST['qno']."' AND t_cash_and_card_sales_sum.cl='".$this->sd['cl']."' AND t_cash_and_card_sales_sum.bc = '".$this->sd['branch']."'
        ";
        ...
        $sql11="SELECT sum(amount) as amount FROM opt_receive_cheque_det WHERE trans_code='69' AND trans_no ='".$_POST['qno']."' AND cl ='".$this->sd['cl']."' AND bc ='".$this->sd['branch']."'  ";
        $query11= $this->db->query($sql11);
        $r_detail['cheque_detail'] = $this->db->query($sql11)->result();

        $sql22="SELECT sum(amount) as amount FROM opt_credit_note_det WHERE trans_code='69' AND trans_no ='".$_POST['qno']."' AND cl ='".$this->sd['cl']."' AND bc ='".$this->sd['branch']."'  ";
        $query22= $this->db->query($sql22);
        $r_detail['other1'] = $this->db->query($sql22)->result();

        $sql33="SELECT sum(amount) as amount FROM opt_debit_note_det WHERE trans_code='69' AND trans_no ='".$_POST['qno']."' AND cl ='".$this->sd['cl']."' AND bc ='".$this->sd['branch']."'  ";
        $query33= $this->db->query($sql33);
        $r_detail['other2'] = $this->db->query($sql33)->result();

        $sql_free="SELECT price*qty AS price  FROM t_cash_and_card_sales_det WHERE is_free='1' AND cl='".$this->sd['cl']."' AND bc='".$this->sd['branch']."' AND nno='".$_POST['qno']."'";
        ...
        $this->db->WHERE('t_serial_movement_out.trans_type','69');
        ...
        $this->db->from('t_cash_and_card_sales_sum');
        $this->db->join('t_cash_and_card_sales_det', 't_cash_and_card_sales_det.nno=t_cash_and_card_sales_sum.nno');
        $this->db->where('t_cash_and_card_sales_sum.cl', $this->sd['cl']);
        $this->db->where('t_cash_and_card_sales_sum.bc', $this->sd['branch']);
        $this->db->where('t_cash_and_card_sales_sum.nno', $_POST['qno']);
        $r_detail['amount'] = $this->db->get()->result();

        $this->db->select(array('t_cash_and_card_sales_additional_item.type', 't_cash_and_card_sales_additional_item.amount', 'r_additional_item.description', 'r_additional_item.is_add'));
        $this->db->from('t_cash_and_card_sales_additional_item');
        $this->db->join('r_additional_item', 't_cash_and_card_sales_additional_item.type=r_additional_item.code');
        $this->db->where('t_cash_and_card_sales_additional_item.cl', $this->sd['cl']);
        $this->db->where('t_cash_and_card_sales_additional_item.bc', $this->sd['branch']);
        $this->db->where('t_cash_and_card_sales_additional_item.nno', $_POST['qno']);
        ...
        $r_detail['discount'] = $this->db->get('t_cash_and_card_sales_sum')->result();
        ...
        $this->db->where('bc', $this->sd['branch']);
        $this->db->where('nno', $_POST['qno']);
        $r_detail['cash_sum'] = $this->db->get('t_cash_and_card_sales_sum')->result();
    ...
    public function pending_special_sales(){
        $cl=$this->sd['cl'];
        $bc=$this->sd['branch'];
        $sql="SELECT * FROM t_cash_and_card_sales_sum WHERE cl='$cl' AND bc='$bc' AND is_approve='0'";
        $query=$this->db->query($sql);
...
public function load_default_customer(){
    ...
    $sql = "SELECT * FROM m_customer  WHERE name LIKE '%$_POST[search]%' OR nic LIKE '%$_POST[search]%'  OR code LIKE '%$_POST[search]%' LIMIT 25";
      ...
      $a .= "<th class='tb_head_th'>Customer NIC</th>";
      $a .= "</thead></tr><tr class='cl'><td>&nbsp;</td><td colspan='2'>&nbsp;</td></tr>";
