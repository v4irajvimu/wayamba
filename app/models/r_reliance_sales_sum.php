...
class r_reliance_sales_sum extends CI_Model{
  ...
  public function PDF_report(){
    ...
    if($_POST['sales_category']!="0"){
      ...
      $query1=$this->db->query('SELECT SUM(`t_credit_sales_sum`.gross_amount) AS gsum,
        ...
        WHERE t_credit_sales_sum.category ="'.$_POST['sales_category'].'" 
        AND `t_credit_sales_sum`.`cl`="'.$this->sd['cl'].'" 
        AND `t_credit_sales_sum`.`bc`= "'.$this->sd['branch'].'"
      ...
      $query1=$this->db->query('SELECT SUM(`t_credit_sales_sum`.gross_amount) AS gsum,
          ...
          LEFT JOIN `t_credit_sales_additional_item` ad 
            ...
            AND `t_credit_sales_sum`.`cl` = `ad`.`cl`
            AND `t_credit_sales_sum`.`bc` = `ad`.`bc`  
        WHERE `t_credit_sales_sum`.`cl`="'.$this->sd['cl'].'" AND `t_credit_sales_sum`.`bc`= "'.$this->sd['branch'].'"
        and `t_credit_sales_sum`.`ddate` between "'.$_POST['from'].'" 
        and "'.$_POST['to'].'"
        AND cus_id = "'.$_POST['r_customer'].'"');
      $r_detail['sum']=$query1->result();
