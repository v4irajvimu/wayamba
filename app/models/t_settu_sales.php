...
class t_settu_sales extends CI_Model {
    ...
    public function base_details(){
        $a['max_no'] = $this->utility->get_max_no("t_seettu_sales_sum","nno");
        return $a;
    }
...
public function validation(){
 $status = 1;
   $this->max_no=$this->utility->get_max_no("t_seettu_sales_sum","nno");
  return $status;
}
   ...
   $this->db->trans_begin();
    ...
    try {
        ...
        if($validation_status == 1){
          $inv_no = $_POST['s_nno'];
          for($x = 0; $x<25; $x++){
            if(isset($_POST['n_'.$x],$_POST['check_'.$x]) ){
              if($_POST['n_'.$x] != "" ){
                ...
                $sum[]=array(
                  ...
                  "route"             =>$_POST['root_id'],
                  "store"             =>$_POST['store_from_id'],
                  "collection_officer"=>$_POST['officer_id'],
                  "memo"              =>$_POST['memo'],
      ...
      if($_POST['hid'] == "0" || $_POST['hid'] == ""){
        if($this->user_permissions->is_add('t_settu_sales')){
          if(count($sum)){
            $this->db->insert_batch("t_seettu_sales_sum",$sum);
          }
          if(count($det)){
            $this->db->insert_batch("t_seettu_sales_det",$det);
          }
          for ($x=0; $x<25; $x++){
        ...
        for ($x=0; $x<25; $x++){
          if (isset($_POST['1_'.$x], $_POST['check_'. $x])) {
            $this->db->where("nno",$_POST['load_id']);
            $this->db->where("cl", $this->sd['cl']);
            $this->db->where("bc", $this->sd['branch']);
            $this->db->where("code", $_POST['5_'.$x]);
            $this->db->update('t_settu_load_det', array("status"=>2));
          }
        }
          ...
          $this->utility->save_logger("SAVE",81,$this->max_no,$this->mod);
        ...
        if($this->user_permissions->is_edit('t_settu_sales')){
...
public function load(){
    ...
    $sql_sum="SELECT  nno,
                      ...
                      load_no,
                      route,
                      r.`description` AS route_des,
                      collection_officer AS officer_id,
                      e.`name` AS officer_name,
                      memo,
                      net_amount,
                      s.`store`,
                      m.`description` AS store_des 
                    FROM `t_seettu_sales_sum` s
                    JOIN r_root r ON r.`code` = s.`route`
                    JOIN m_employee e ON e.`code` = s.`collection_officer` 
                    JOIN m_stores m ON m.`code` = s.`store`
    ...
    $sql_det="SELECT  d.`organizer`,
                      c.`name` AS organizer_name,
                      d.`seettu_no`,
                      d.`category` AS ref_cat,
                      ct.`code` AS category,
                      d.`code`,
                      m.`name`
              FROM t_seettu_sales_det d
              JOIN m_customer c ON c.`code` = d.`organizer`
              JOIN `m_settu_item_sum` m ON m.`code` = d.`code`
              JOIN `m_settu_item_category` ct ON ct.`ref_code` = d.`category`
              WHERE d.cl='".$this->sd['cl']."' 
              AND d.bc='".$this->sd['branch']."' 
              AND d.`nno`='".$_POST['no']."'";
  ...
  public function delete() {
        ...
        set_error_handler('exceptionThrower');
        try {
            if($this->user_permissions->is_delete('t_settu_sales')){
              ...
              $this->db->where("cl", $this->sd['cl']);
              $this->db->where("bc", $this->sd['branch']);
              $this->db->update('t_seettu_sales_sum',array("is_cancel" => 1));
