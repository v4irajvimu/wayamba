...
class r_nationality extends CI_Model {
	...
	$this->mtb = $this->tables->tb['r_nationality'];
    ...
    $a['max_no'] = $this->utility->is_auto_genarate("r_nationality","max_no","is_auto_genarate_nationality","nationality_code_type");
	...
	//	$this->db->join($this->tb_main_regeon, $this->tb_main_regeon.'.code = '.$this->mtb.'.region', 'INNER');
        ...
        foreach($query->result() as $r){
            ...
            if($this->user_permissions->is_delete('r_nationality')){$but .= "<img src='".base_url()."img/delete.gif' onclick='set_delete(\"".$r->code."\")' title='Delete' />";}
        ...
        try { 
    	    ...
    	    if(isset($_POST['code_'])){
                if($_POST['code_'] == "0" || $_POST['code_'] == ""){
                    if($this->user_permissions->is_add('r_nationality')){
                        $arr=array(
                            "code"=>$_POST['code'],
                            "description"=>$_POST['description'],
                            "oc"=>$this->sd['oc'],
                            "max_no"=>$this->utility->max_nno("r_nationality","max_no"),
                        );
                    ...
                    if($this->user_permissions->is_edit('r_nationality')){
	...
	//echo '<pre>'.print_r($this->db->get($this->mtb)->first_row(),true).'</pre>';
    ...
    public function delete_validation(){
        $status=1;
        $codes=$_POST['code'];
        $check_cancellation = $this->utility->check_account_trans($codes,'nationality','r_nationality','m_customer','nationality');
    ...
    public function delete(){
        ...
        set_error_handler('exceptionThrower'); 
        try {
            if($this->user_permissions->is_delete('r_nationality')){
    ...
    public function select($type = 'code', $style = ''){
        ...
        $s = "<select name='nationality' id='nation' style='".$style."'>";
