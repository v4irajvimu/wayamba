...
class r_hp_closed_accounts extends CI_Model{
  ...
  public function PDF_report(){
    ...
    $sql="SELECT SUM(`t_hp_sales_sum`.`net_amount`) as net_sum , sum(`t_hp_sales_sum`.`down_payment`)  as downp_sum, sum(`t_hp_sales_sum`.`interest_amount`) as interest_sum, sum(`t_hp_sales_sum`.`document_charges`) as other_char_sum, sum(`t_hp_sales_sum`.`installment_amount`) as inst_sum, sum(`t_hp_sales_sum`.`no_of_installments`) as no_of_inst_sum 
    FROM `t_hp_sales_sum`
    ...
    WHERE `t_hp_sales_sum`.`bc`= '".$this->sd['branch']."'
    and `t_hp_sales_sum`.`ddate` between '".$_POST['from']."' and '".$_POST['to']."' AND `t_hp_sales_sum`.is_closed='1'";
