...
class r_downpayment_rec_list extends CI_Model{
  ...
  public function PDF_report(){
    ...
    $sql="SELECT s.ddate,
                ...
                d.`paid` AS down_amount
        FROM t_hp_receipt_sum s
        JOIN m_customer c ON c.`code` = s.`customer`
        JOIN t_hp_receipt_det d ON d.`cl` = s.`cl` AND d.`bc` = s.`bc` AND d.`nno` = s.`nno`
        WHERE s.ddate BETWEEN '".$_POST['from']."' AND '".$_POST['to']."' 
            AND d.`order_type` ='5' AND s.is_cancel='0'
