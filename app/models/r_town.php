...
class r_town extends CI_Model {
	...
	$this->mtb = $this->tables->tb['r_town'];
    ...
    $a['max_no'] = $this->utility->is_auto_genarate("r_town","max_no","is_auto_genarate_town","town_code_type");
	...
	//	$this->db->join($this->tb_main_regeon, $this->tb_main_regeon.'.code = '.$this->mtb.'.region', 'INNER');
        ...
        foreach($query->result() as $r){
            ...
            if($this->user_permissions->is_delete('r_town')){$but .= "<img src='".base_url()."img/delete.gif' onclick='set_delete(\"".$r->code."\")' title='Delete' />";}
        ...
        try {
    	    ...
    	    if(isset($_POST['code_'])){
                if($_POST['code_'] == "0" || $_POST['code_'] == ""){
                    if($this->user_permissions->is_add('r_town')){
                        $arr=array(
                            "code"=>$_POST['code'],
                            "description"=>$_POST['description'],
                            "oc"=>$this->sd['oc'],
                            "max_no"=>$this->utility->max_nno("r_town","max_no"),
                        );
                    ...
                    if($this->user_permissions->is_edit('r_town')){
    ...
    public function delete_validation(){
        $status=1;
        $codes=$_POST['code'];
        $check_cancellation = $this->utility->check_account_trans($codes,'town','r_town','m_customer','town');
    ...
    public function delete(){
        ...
        set_error_handler('exceptionThrower'); 
        try {
            if($this->user_permissions->is_delete('r_town')){
    ...
    public function select($type = 'code', $style = ''){
        ...
        $s = "<select name='town' id='town' style='".$style."'>";
