...
class r_cancelled_voucher_lists extends CI_Model{
    ...
    public function PDF_report(){
      ...
      $sql="SELECT nno,
                  ...
                  settle_amount,
                  pay_cash,
                  CONCAT(s.cl,'',s.bc,'',LPAD(s.nno,5,'0')) as voucher_no,
                  pay_issue_chq,
                  pay_credit,
                  pay_dnote,
                  pay_discount,
                  memo,
                  CONCAT(s.acc_code,' - ', a.description) as acc_code
          FROM  t_voucher_sum s
  ...
  if(!empty($_POST['from']))
    {
     $sql.=" AND s.ddate between '".$_POST['from']."' AND '".$_POST['to']."'";
   }
   if(!empty($_POST['acc_code']))
   {
     $sql.=" AND s.acc_code = '$acc'";
   }
  ...
  {
   $sql.=" AND s.`nno` >= '$no_range_frm'";
 }

 if(!empty($_POST['t_range_to']) && empty($_POST['t_range_from']))
 {
   $sql.=" AND s.`nno` <= '$no_range_to'";
 }

 if(!empty($_POST['t_range_to']) && !empty($_POST['t_range_to']))
 {
   $sql.=" AND s.`nno` BETWEEN  '$no_range_frm' AND '$no_range_to'";
 }

 $sql.="ORDER BY s.cl,s.bc,s.nno ASC";
